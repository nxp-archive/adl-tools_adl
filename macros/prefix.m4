#
# Source:  http://ac-archive.sourceforge.net/guidod/ax_prefix_config_h.html
#
...
# e.g. "#define const", "#define restrict", or "#define off_t", (and this one can
# live in another directory, e.g. testpkg/config.h therefore I decided to move the
# output-header to be the first arg)

# takes the usual config.h generated header file; looks for each of the generated
# "#define SOMEDEF" lines, and prefixes the defined name (ie. makes it "#define
# PREFIX_SOMEDEF". The result is written to the output config.header file. The
# PREFIX is converted to uppercase for the conversions.
...
# Your configure.ac script should contain both macros in this order, and unlike
# the earlier variations of this prefix-macro it is okay to place the
...
# if the argument to AX_PREFIX_CONFIG_H would have been omitted then the default
# outputfile would have been called simply "testpkg-config.h", but even under the
# name "mylib/_config.h" it contains prefix-defines like
...
# and this "mylib/_config.h" can be installed along with other header-files, which
# is most convenient when creating a shared library (that has some headers) where
# some functionality is dependent on the OS-features detected at compile-time. No
# need to invent some "mylib-confdefs.h.in" manually. :-)

# Note that some AC_DEFINEs that end up in the config.h file are actually
# self-referential - e.g. AC_C_INLINE, AC_C_CONST, and the AC_TYPE_OFF_T say that
# they "will define inline|const|off_t if the system does not do it by
# itself". You might want to clean up about these - consider an extra mylib/conf.h
...
# and then start using _testpkg_const in the header files. That is also a good
# thing to differentiate whether some library-user has starting to take up with a
# different compiler, so perhaps it could read something like this:
