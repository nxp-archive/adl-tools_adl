...
namespace adliss {
  ...
  struct P : public IssCore, public InstrCountBase {
    ...
    void enable_rollback(bool e) { _rollback_enable = e; }
    
    void commit_rollback() { _restoreStack.commit(); }
    
    void flush_rollback() { _restoreStack.reset(); }
    
    RestoreStack get_rollback_data() {
      RestoreStack tmp;
      tmp.set(_restoreStack);
      return tmp;
    }
    ...
    struct LogmemMemoryWrite {
      ...
      void operator()(bool pre,int seq,addr_t ea,addr_t ra,int num_bytes,uint64_t value) {
        if (pre && _core._rollback_enable) {
          bits<32> value = mem.read32( ra & ~WordMask);
          typedef void (P::*Functor) (addr_t,const bits<32>&);
          Functor pCommit = &P::mem_restore_write;
          static PacketPool<CommitMemWrite<P*,Functor,32,100>, 100>   pool;
          CommitMemWrite<P*,Functor,32,100> *cmt = pool.alloc();
          cmt->reset(&_core,pCommit,(ra&~WordMask),value,pool);
          _core._restoreStack.add(cmt);
        }
    ...
    struct dmi_read_internal<Core,Translate,CacheAccess,Logger,8,Interleaved> {
      mem_bits<64> operator()(Core &core,Translate translate,CacheAccess cache,Logger logger,addr_t ea) const  ATTRIBUTE_USED 
      {
        ...
        if (  (((ea ^ core._dmi_mem_read._items[table_index]._ea) & (DmiCache::CompMask | 7)) == 0)) {
          core.addr_check_handler(ea,make_addr(core._dmi_mem_read._items[table_index]._ra,DmiCache::CompMask,ea),LoadTrans);
    ...
    void update_dmi_mem_read(addr_t ea,addr_t ra,byte_t *p) {
      ...
      addr_t ra_page = ra & DmiCache::CompMask;
      _dmi_mem_read._items[table_index]._ra = ra_page;
    }
    ...
    struct dmi_write_internal<Core,Translate,CacheAccess,Logger,8,Nb,Interleaved> {
      void operator()(Core &core,Translate translate,CacheAccess cache,Logger logger,addr_t ea,mem_bits<Nb> value) const ATTRIBUTE_USED 
      {
        ...
        if ( (((ea ^ core._dmi_mem_write._items[table_index]._ea) & (DmiCache::CompMask | 7)) == 0)) {
          core.addr_check_handler(ea,make_addr(core._dmi_mem_write._items[table_index]._ra,DmiCache::CompMask,ea),StoreTrans);
    ...
    void update_dmi_mem_write(addr_t ea,addr_t ra,byte_t *p) {
      ...
      addr_t ra_page = ra & DmiCache::CompMask;
      _dmi_mem_write._items[table_index]._ra = ra_page;
    }
    ...
    template <size_t NB>
     bits<NB*8> mem_read_jit (addr_t ea,int type = CacheRead,bool has_cw_addr = false,addr_t cw_addr = 0)
    {
    ...
    template <size_t NB>
     void mem_write_jit (addr_t ea,bits<NB*8> value,int type = CacheWrite,bool has_cw_addr = false,addr_t cw_addr = 0)
    {
    ...
    void mem_write_jit (const bits<Nb1> &addr,const bits<Nb2> &value,int type = CacheWrite) {
      mem_write_jit<NB>(addr.uint64(),value,type,false,0);
    };
    
    template <size_t NB,size_t Nb>
    bits<NB*8> mem_read_jit (bits<Nb> addr,int type = CacheRead) {
      return mem_read_jit<NB>(addr.uint64(),type,false,0);
    };
    
    
    template <size_t NB,size_t Nb1,size_t Nb2,size_t Nb3>
    void mem_write_jit (const bits<Nb1> &addr,const bits<Nb2> &value,int type = CacheWrite,bool has_cw_addr = false,const bits<Nb3> &cw_addr = 0) {
      mem_write_jit<NB>(addr.uint64(),value,type,has_cw_addr,cw_addr.uint64());
    };
    
    template <size_t NB,size_t Nb,size_t Nb3>
    bits<NB*8> mem_read_jit (bits<Nb> addr,int type = CacheRead,bool has_cw_addr = false,const bits<Nb3> &cw_addr = 0) {
      return mem_read_jit<NB>(addr.uint64(),type,has_cw_addr,cw_addr.uint64());
    };
    ...
    void mem_restore_write(addr_t ra,const bits<32> &x) {
      mem.write32(ra,x.uint32(),4);
    }
    ...
    void AR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = AR(foobar._foobar_active).get_data();
        decltype(&P::AR_restore) pCommit = &P::AR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
      }
    }
    
    void AR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      AR(foobar._foobar_active).set_data() = x;
    }
    ...
    bits<8> AR_C_read(unsigned i)  ATTRIBUTE_USED {
      return AR_read(). get<8>(((i%32)*8),(((i%32)*8)+8-1));
    };
    ...
    void AR_R_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = AR_R().get_data();
        decltype(&P::AR_R_restore) pCommit = &P::AR_R_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void AR_R_restore( const bits<32> &x)  ATTRIBUTE_USED {
      AR_R().set_data() = x;
    }
    ...
    void AR_W_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = AR_W().get_data();
        decltype(&P::AR_W_restore) pCommit = &P::AR_W_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void AR_W_restore( const bits<32> &x)  ATTRIBUTE_USED {
      AR_W().set_data() = x;
    }
    ...
    void BCCNT_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = BCCNT().get_data();
        decltype(&P::BCCNT_restore) pCommit = &P::BCCNT_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void BCCNT_restore( const bits<32> &x)  ATTRIBUTE_USED {
      BCCNT().set_data() = x;
    }
    ...
    void BR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = BR(foobar._foobar_active).get_data();
        decltype(&P::BR_restore) pCommit = &P::BR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void BR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      BR(foobar._foobar_active).set_data() = x;
    }
    bits<16> BR_B_read()  ATTRIBUTE_USED {
      return BR_read(). get<16>(16,31);
    };
    ...
    void BR_C_write(unsigned i,const bits<8> &x)  ATTRIBUTE_USED {
      BR_save();
      BR_read().set<8>(((i%32)*8),(((i%32)*8)+8-1),x);
      CondRegTrace(BR_trace());
    };
    ...
    void BranchTaken_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = BranchTaken().get_data();
        decltype(&P::BranchTaken_restore) pCommit = &P::BranchTaken_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void BranchTaken_restore( const bits<32> &x)  ATTRIBUTE_USED {
      BranchTaken().set_data() = x;
    }
    ...
    void CIA_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = CIA().get_data();
        decltype(&P::CIA_restore) pCommit = &P::CIA_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void CIA_restore( const bits<32> &x)  ATTRIBUTE_USED {
      CIA().set_data() = x;
    }
    ...
    void CR_cr_write(unsigned i,const bits<4> &x)  ATTRIBUTE_USED {
      CR_read().set<4>(((i%32)*4),(((i%32)*4)+4-1),x);
    ...
    void CSRR0_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = CSRR0().get_data();
        decltype(&P::CSRR0_restore) pCommit = &P::CSRR0_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void CSRR0_restore( const bits<32> &x)  ATTRIBUTE_USED {
      CSRR0().set_data() = x;
    }
    ...
    void CSRR1_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = CSRR1().get_data();
        decltype(&P::CSRR1_restore) pCommit = &P::CSRR1_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void CSRR1_restore( const bits<32> &x)  ATTRIBUTE_USED {
      CSRR1().set_data() = x;
    }
    ...
    void CTR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = CTR().get_data();
        decltype(&P::CTR_restore) pCommit = &P::CTR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void CTR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      CTR().set_data() = x;
    }
    ...
    void DEC_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = DEC().get_data();
        decltype(&P::DEC_restore) pCommit = &P::DEC_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void DEC_restore( const bits<32> &x)  ATTRIBUTE_USED {
      DEC().set_data() = x;
    }
    ...
    void DR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = DR(foobar._foobar_active).get_data();
        decltype(&P::DR_restore) pCommit = &P::DR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void DR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      DR(foobar._foobar_active).set_data() = x;
    }
    ...
    void ESR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = ESR().get_data();
        decltype(&P::ESR_restore) pCommit = &P::ESR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void ESR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      ESR().set_data() = x;
    }
    ...
    void EWTEST_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = EWTEST().get_data();
        decltype(&P::EWTEST_restore) pCommit = &P::EWTEST_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void EWTEST_restore( const bits<32> &x)  ATTRIBUTE_USED {
      EWTEST().set_data() = x;
    }
    ...
    void FCNT_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FCNT().get_data();
        decltype(&P::FCNT_restore) pCommit = &P::FCNT_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FCNT_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FCNT().set_data() = x;
    }
    ...
    void FOO1_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO1().get_data();
        decltype(&P::FOO1_restore) pCommit = &P::FOO1_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO1_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO1().set_data() = x;
    }
    bits<8> FOO1_A_read()  ATTRIBUTE_USED {
      return FOO1_read(). get<8>(0,7);
    };
    ...
    void FOO1_C_write_delayed(const bits<8> &x,unsigned d)  ATTRIBUTE_USED {
      ...
      static PacketPool<CommitRegFieldWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
      CommitRegFieldWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
      cmt->reset(this,pCommit,x,pool);
      _commitQueue.add(cmt,d);
    }
    ...
    void FOO11_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO11().get_data();
        decltype(&P::FOO11_restore) pCommit = &P::FOO11_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO11_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO11().set_data() = x;
    }
    ...
    void FOO13_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO13().get_data();
        decltype(&P::FOO13_restore) pCommit = &P::FOO13_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO13_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO13().set_data() = x;
    }
    
    // Register FOO14
    void FOO14_trace()  ATTRIBUTE_USED {
      if (Tracing && (_trace_mode & ttRegWrite)) {
        _logger->log_reg_write(24,"FOO14",FOO14().get_data().rnum());
      }
    }
    
    void FOO14_write_internal(const bits<32> &x) {
      FOO14_save();
      FOO14().set_data() = (x & ~FOO14()._reserved_mask);
    };
    void FOO14_write(const bits<32> &x) {
      FOO14_write_internal(x);
      CondRegTrace(FOO14_trace());
    };
    bits<32> & FOO14_read_internal() {
      return FOO14().get_data();
    };
    
    bits<32> & FOO14_read()  ATTRIBUTE_USED {
      return FOO14_read_internal();
    }
    ...
    void FOO14_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO14().get_data();
        decltype(&P::FOO14_restore) pCommit = &P::FOO14_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO14_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO14().set_data() = x;
    }
    bits<1> getfield_FOO14_A(const bits<32> &x) {
      return x. get<1>(0,0);
    }
    
    bits<32> setfield_FOO14_A(bits<32> x,const bits<1> &v) {
      x.set(0,0,v);
      return x;
    }
    
    bits<1> FOO14_A_read()  ATTRIBUTE_USED {
      return FOO14_read(). get<1>(0,0);
    };
    
    void FOO14_A_write(const bits<1> &x)  ATTRIBUTE_USED {
      FOO14_save();
      FOO14_read().set(0,0,x);
      CondRegTrace(FOO14_trace());
    };
    bits<1> getfield_FOO14_B(const bits<32> &x) {
      return x. get<1>(1,1);
    }
    
    bits<32> setfield_FOO14_B(bits<32> x,const bits<1> &v) {
      x.set(1,1,v);
      return x;
    }
    
    bits<1> FOO14_B_read()  ATTRIBUTE_USED {
      return FOO14_read(). get<1>(1,1);
    };
    
    void FOO14_B_write(const bits<1> &x)  ATTRIBUTE_USED {
      FOO14_save();
      FOO14_read().set(1,1,x);
      CondRegTrace(FOO14_trace());
    };
    bits<1> getfield_FOO14_C(const bits<32> &x) {
      return x. get<1>(2,2);
    }
    
    bits<32> setfield_FOO14_C(bits<32> x,const bits<1> &v) {
      x.set(2,2,v);
      return x;
    }
    
    bits<1> FOO14_C_read()  ATTRIBUTE_USED {
      return FOO14_read(). get<1>(2,2);
    };
    
    void FOO14_C_write(const bits<1> &x)  ATTRIBUTE_USED {
      FOO14_save();
      FOO14_read().set(2,2,x);
      CondRegTrace(FOO14_trace());
    };
    bits<1> getfield_FOO14_D(const bits<32> &x) {
      return x. get<1>(3,3);
    }
    
    bits<32> setfield_FOO14_D(bits<32> x,const bits<1> &v) {
      x.set(3,3,v);
      return x;
    }
    
    bits<1> FOO14_D_read()  ATTRIBUTE_USED {
      return FOO14_read(). get<1>(3,3);
    };
    
    void FOO14_D_write(const bits<1> &x)  ATTRIBUTE_USED {
      FOO14_save();
      FOO14_read().set(3,3,x);
      CondRegTrace(FOO14_trace());
    };
    template <size_t Nb>
    void FOO14_slice_set_internal(unsigned l,unsigned r,const bits<Nb> &x) {
      FOO14_save();
      FOO14_read().set(l,r,(x & ~FOO14()._reserved_mask(l,r)));
    };
    template <size_t Nb>
    void FOO14_slice_set(unsigned l,unsigned r,const bits<Nb> &x) {
      FOO14_slice_set_internal(l,r,x);
      CondRegTrace(FOO14_trace());
    }
    
    void FOO14_slice_set(unsigned l,unsigned r,uint64_t x) {
      FOO14_slice_set(l,r,bits<32>(x));
    };
    ...
    void FOO2_write_delayed(const bits<64> &x,unsigned d) {
      ...
      static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),64,100>, 100>   pool;
      CommitRegWrite<decltype(this),decltype(pCommit),64,100> *cmt = pool.alloc();
      cmt->reset(this,pCommit,x,pool);
      _commitQueue.add(cmt,d);
    }
    ...
    void FOO2_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<64> x = FOO2().get_data();
        decltype(&P::FOO2_restore) pCommit = &P::FOO2_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),64,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),64,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO2_restore( const bits<64> &x)  ATTRIBUTE_USED {
      FOO2().set_data() = x;
    }
    bits<8> FOO2_X_read(unsigned i)  ATTRIBUTE_USED {
      return FOO2_read(). get<8>(((i%64)*8),(((i%64)*8)+8-1));
    };
    ...
    void FOO2_X_write_delayed(unsigned i,const bits<8> &x,unsigned d)  ATTRIBUTE_USED {
      ...
      static PacketPool<CommitRegIndexedFieldWrite<decltype(this),decltype(pCommit),64,100>, 100>   pool;
      CommitRegIndexedFieldWrite<decltype(this),decltype(pCommit),64,100> *cmt = pool.alloc();
    ...
    void FOO2_X_commit_write(unsigned i,const bits<8> &x)  ATTRIBUTE_USED {
      FOO2_save();
      FOO2_read().set<8>(((i%64)*8),(((i%64)*8)+8-1),x);
      CondRegTrace(FOO2_trace());
    };
    bits<16> FOO2_Y_read(unsigned i)  ATTRIBUTE_USED {
      return FOO2_read(). get<16>(((i%64)*16),(((i%64)*16)+16-1));
    };
    ...
    void FOO2_Y_commit_write(unsigned i,const bits<16> &x)  ATTRIBUTE_USED {
      FOO2_save();
      FOO2_read().set<16>(((i%64)*16),(((i%64)*16)+16-1),x);
      CondRegTrace(FOO2_trace());
    };
    bits<32> FOO2_Z_read(unsigned i)  ATTRIBUTE_USED {
      return FOO2_read(). get<32>(((i%64)*32),(((i%64)*32)+32-1));
    };
    ...
    void FOO2_Z_commit_write(unsigned i,const bits<32> &x)  ATTRIBUTE_USED {
      FOO2_save();
      FOO2_read().set<32>(((i%64)*32),(((i%64)*32)+32-1),x);
      CondRegTrace(FOO2_trace());
    };
    ...
    void FOO3_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO3().get_data();
        decltype(&P::FOO3_restore) pCommit = &P::FOO3_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO3_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO3().set_data() = x;
    }
    ...
    void FOO5_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO5().get_data();
        decltype(&P::FOO5_restore) pCommit = &P::FOO5_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO5_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO5().set_data() = x;
    }
    bits<16> FOO5_A_read()  ATTRIBUTE_USED {
      return FOO5_read(). get<16>(0,15);
    };
    ...
    void FOO5_RD_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO5_RD().get_data();
        decltype(&P::FOO5_RD_restore) pCommit = &P::FOO5_RD_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO5_RD_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO5_RD().set_data() = x;
    }
    ...
    void FOO5_WR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO5_WR().get_data();
        decltype(&P::FOO5_WR_restore) pCommit = &P::FOO5_WR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO5_WR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO5_WR().set_data() = x;
    }
    ...
    void FOO7_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO7().get_data();
        decltype(&P::FOO7_restore) pCommit = &P::FOO7_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO7_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO7().set_data() = x;
    }
    ...
    void FOO9_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = FOO9().get_data();
        decltype(&P::FOO9_restore) pCommit = &P::FOO9_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void FOO9_restore( const bits<32> &x)  ATTRIBUTE_USED {
      FOO9().set_data() = x;
    }
    template <size_t Nb>
    ...
    void ICNT_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = ICNT().get_data();
        decltype(&P::ICNT_restore) pCommit = &P::ICNT_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void ICNT_restore( const bits<32> &x)  ATTRIBUTE_USED {
      ICNT().set_data() = x;
    }
    ...
    void MSR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = MSR().get_data();
        decltype(&P::MSR_restore) pCommit = &P::MSR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void MSR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      MSR().set_data() = x;
      {
        _sym651();
      }
    ...
    void NIA_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = NIA().get_data();
        decltype(&P::NIA_restore) pCommit = &P::NIA_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void NIA_restore( const bits<32> &x)  ATTRIBUTE_USED {
      NIA().set_data() = x;
    }
    ...
    void PCCNT_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = PCCNT().get_data();
        decltype(&P::PCCNT_restore) pCommit = &P::PCCNT_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void PCCNT_restore( const bits<32> &x)  ATTRIBUTE_USED {
      PCCNT().set_data() = x;
    }
    ...
    void POSTEXCPT_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = POSTEXCPT().get_data();
        decltype(&P::POSTEXCPT_restore) pCommit = &P::POSTEXCPT_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void POSTEXCPT_restore( const bits<32> &x)  ATTRIBUTE_USED {
      POSTEXCPT().set_data() = x;
    }
    ...
    void PREEXCPT_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = PREEXCPT().get_data();
        decltype(&P::PREEXCPT_restore) pCommit = &P::PREEXCPT_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void PREEXCPT_restore( const bits<32> &x)  ATTRIBUTE_USED {
      PREEXCPT().set_data() = x;
    }
    ...
    void SRR0_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = SRR0().get_data();
        decltype(&P::SRR0_restore) pCommit = &P::SRR0_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void SRR0_restore( const bits<32> &x)  ATTRIBUTE_USED {
      SRR0().set_data() = x;
    }
    ...
    void SRR1_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = SRR1().get_data();
        decltype(&P::SRR1_restore) pCommit = &P::SRR1_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void SRR1_restore( const bits<32> &x)  ATTRIBUTE_USED {
      SRR1().set_data() = x;
    }
    ...
    void XER_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = XER().get_data();
        decltype(&P::XER_restore) pCommit = &P::XER_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void XER_restore( const bits<32> &x)  ATTRIBUTE_USED {
      XER().set_data() = x;
    }
    ...
    void XER_slice_set_delayed(unsigned l,unsigned r,const bits<Nb> &x,unsigned d) {
      ...
      static PacketPool<CommitRegSliceSet<decltype(this),decltype(pCommit),32,100>, 100>   pool;
      CommitRegSliceSet<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
    ...
    void tmpESR_save()  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = tmpESR().get_data();
        decltype(&P::tmpESR_restore) pCommit = &P::tmpESR_restore;
        static PacketPool<CommitRegWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,x,pool);
        _restoreStack.add(cmt);
    ...
    void tmpESR_restore( const bits<32> &x)  ATTRIBUTE_USED {
      tmpESR().set_data() = x;
    }
    template <size_t Nb>
    void tmpESR_slice_set_internal(unsigned l,unsigned r,const bits<Nb> &x) {
      tmpESR_save();
      tmpESR_read().set(l,r,x);
    };
    template <size_t Nb>
    ...
    void ARF_save(unsigned index)  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = ARF(foobar._foobar_active)(index).get_data();
        decltype(&P::ARF_restore) pCommit = &P::ARF_restore;
        static PacketPool<CommitRegfileWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegfileWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,index,x,pool);
        _restoreStack.add(cmt);
      }
    }
    
    void ARF_restore(unsigned index, const bits<32> &x)  ATTRIBUTE_USED {
      ARF(foobar._foobar_active)(index).set_data() = x;
    }
    ...
    bits<16> ARF_B_read(unsigned index)  ATTRIBUTE_USED {
      return ARF_read(index). get<16>(16,31);
    }
    ...
    bits<8> ARF_C_read(unsigned index,unsigned i)  ATTRIBUTE_USED {
      return ARF_read(index).get<8>(((i%32)*8),(((i%32)*8)+8-1));
    };
    ...
    void CRF_save(unsigned index)  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = CRF(foobar._foobar_active)(index).get_data();
        decltype(&P::CRF_restore) pCommit = &P::CRF_restore;
        static PacketPool<CommitRegfileWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegfileWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,index,x,pool);
        _restoreStack.add(cmt);
    ...
    void CRF_restore(unsigned index, const bits<32> &x)  ATTRIBUTE_USED {
      CRF(foobar._foobar_active)(index).set_data() = x;
    }
    ...
    void GPR_save(unsigned index)  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = GPR()(index).get_data();
        decltype(&P::GPR_restore) pCommit = &P::GPR_restore;
        static PacketPool<CommitRegfileWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegfileWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,index,x,pool);
        _restoreStack.add(cmt);
    ...
    void GPR_restore(unsigned index, const bits<32> &x)  ATTRIBUTE_USED {
      GPR()(index).set_data() = x;
    }
    ...
    void GPR_slice_set_delayed(unsigned index, unsigned l,unsigned r,const bits<Nb> &x,unsigned d) {
      ...
      static PacketPool<CommitRegfileSliceSet<decltype(this),decltype(pCommit),32,100>, 100>   pool;
      CommitRegfileSliceSet<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
    ...
    void VPR_write_delayed(unsigned index,const bits<128> &x,unsigned d)  ATTRIBUTE_USED {
      ...
      static PacketPool<CommitRegfileWrite<decltype(this),decltype(pCommit),128,100>, 100>   pool;
      CommitRegfileWrite<decltype(this),decltype(pCommit),128,100> *cmt = pool.alloc();
      cmt->reset(this,pCommit,index,x,pool);
      _commitQueue.add(cmt,d);
    }
    ...
    void VPR_save(unsigned index)  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<128> x = VPR()(index).get_data();
        decltype(&P::VPR_restore) pCommit = &P::VPR_restore;
        static PacketPool<CommitRegfileWrite<decltype(this),decltype(pCommit),128,100>, 100>   pool;
        CommitRegfileWrite<decltype(this),decltype(pCommit),128,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,index,x,pool);
        _restoreStack.add(cmt);
    ...
    void VPR_restore(unsigned index, const bits<128> &x)  ATTRIBUTE_USED {
      VPR()(index).set_data() = x;
    }
    bits<8> VPR_B_read(unsigned index,unsigned i)  ATTRIBUTE_USED {
      return VPR_read(index).get<8>(((i%128)*8),(((i%128)*8)+8-1));
    };
    ...
    void VPR_B_write_delayed(unsigned index, unsigned i,const bits<8> &x,unsigned d)  ATTRIBUTE_USED {
      ...
      static PacketPool<CommitRegfileIndexedFieldWrite<decltype(this),decltype(pCommit),128,100>, 100>   pool;
      CommitRegfileIndexedFieldWrite<decltype(this),decltype(pCommit),128,100> *cmt = pool.alloc();
    ...
    void VPR_B_commit_write(unsigned index, unsigned i,const bits<8> &x)  ATTRIBUTE_USED {
      VPR_save(index);
      VPR_read(index).set<8>(((i%128)*8),(((i%128)*8)+8-1),x);
      CondRegTrace(VPR_trace(index));
    };
    bits<16> VPR_H_read(unsigned index,unsigned i)  ATTRIBUTE_USED {
      return VPR_read(index).get<16>(((i%128)*16),(((i%128)*16)+16-1));
    };
    ...
    void VPR_H_commit_write(unsigned index, unsigned i,const bits<16> &x)  ATTRIBUTE_USED {
      VPR_save(index);
      VPR_read(index).set<16>(((i%128)*16),(((i%128)*16)+16-1),x);
      CondRegTrace(VPR_trace(index));
    };
    bits<32> VPR_W_read(unsigned index,unsigned i)  ATTRIBUTE_USED {
      return VPR_read(index).get<32>(((i%128)*32),(((i%128)*32)+32-1));
    };
    ...
    void VPR_W_commit_write(unsigned index, unsigned i,const bits<32> &x)  ATTRIBUTE_USED {
      VPR_save(index);
      VPR_read(index).set<32>(((i%128)*32),(((i%128)*32)+32-1),x);
      CondRegTrace(VPR_trace(index));
    };
    ...
    void VPR_slice_set_delayed(unsigned index, unsigned l,unsigned r,const bits<Nb> &x,unsigned d) {
      ...
      static PacketPool<CommitRegfileSliceSet<decltype(this),decltype(pCommit),128,100>, 100>   pool;
      CommitRegfileSliceSet<decltype(this),decltype(pCommit),128,100> *cmt = pool.alloc();
      cmt->reset(this,pCommit,index, l,r,x,pool);
      _commitQueue.add(cmt,d);
    }
    ...
    void XPR_save(unsigned index)  ATTRIBUTE_USED {
      if (_rollback_enable) {
        bits<32> x = XPR()(index).get_data();
        decltype(&P::XPR_restore) pCommit = &P::XPR_restore;
        static PacketPool<CommitRegfileWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
        CommitRegfileWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
        cmt->reset(this,pCommit,index,x,pool);
        _restoreStack.add(cmt);
    ...
    void XPR_restore(unsigned index, const bits<32> &x)  ATTRIBUTE_USED {
      XPR()(index).set_data() = x;
    }
    bits<16> XPR_A_read(unsigned index)  ATTRIBUTE_USED {
      return XPR_read(index). get<16>(0,15);
    }
    ...
    void XPR_A_write_delayed(unsigned index, const bits<16> &x,unsigned d)  ATTRIBUTE_USED {
      ...
      static PacketPool<CommitRegfileFieldWrite<decltype(this),decltype(pCommit),32,100>, 100>   pool;
      CommitRegfileFieldWrite<decltype(this),decltype(pCommit),32,100> *cmt = pool.alloc();
      cmt->reset(this,pCommit,index, x,pool);
      _commitQueue.add(cmt,d);
    }
    ...
    #else
    {
      ...
      try {
        switch (rindex) {
          ...
          case 24:
          if (Tracing && (_trace_mode & (ttRegWrite))) {
            _logger->log_reg_ext_write(24,"FOO14",value);
          }
          FOO14_write(value);
          break;
          case 25:
    ...
    struct DcDecodeItem : public TDcDecodeItem<P,DcBasicBlock,uint32_t,6>
    {
      typedef TDcDecodeItem<P,DcBasicBlock,uint32_t,6> Base;
      DcDecodeItem() {};
      DcDecodeItem(DecodeFunc f) : TDcDecodeItem<P,DcBasicBlock,uint32_t,6>(f) {};
      using Base::set_arg;
    ...
    typedef TBasicBlockHash<P,DcBasicBlock,DcDecodeCache,DcTranslate,2,10,Exception> DcBasicBlockHash;
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_9(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_addi(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym39(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<16> SI)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_12(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_addic_(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym45(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<16> SI)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_15(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_addis(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym51(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<16> SI)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym57(DcBasicBlock *bb,addr_t pc,unsigned count, bits<24> LI_p)
    {
    ...
    // Instruction "bc"
     bool _sym59(bits<5> BO, bits<5> BI, bits<32> BD)
     {
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_21(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      bool rc = core->_sym59(dci->_args[1], dci->_args[2], dci->_args[3]);
      ExtCondTrace({
        if (Tracing && (core->trace_mode() & ttBranchTaken)) {
          core->get_logger()->log_branch_end(core->CIA_read().uint64());
        }
    ...
    inline void _sym63(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> BO, bits<5> BI, bits<14> BD_p)
    {
    ...
    // Instruction "bc2"
     bool _sym65(bits<5> BO, bits<5> BI, bits<32> BD)
     {
      bool __branch_taken__ = false;
      {
             AdlTypeof( ( BO ( 0 ) != 0 ) || ( ( CR_read() ( BI ) == BO ( 1 ) ) != 0 ) ) cond_ok = ( BO ( 0 ) != 0 ) || ( ( CR_read() ( BI ) == BO ( 1 ) ) != 0 ) ;
    ...
    // Decode-cache handler for instruction "bc2".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_24(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"bc2",core->instr_disasm_bc2,ifBrCond);
        }
        if (Tracing && (core->trace_mode() & ttBasicBlockEnd)) {
          core->get_logger()->log_basic_block_end(core->CIA_read().uint64());
        }
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      bool rc = core->_sym65(dci->_args[1], dci->_args[2], dci->_args[3]);
      ExtCondTrace({
        if (Tracing && (core->trace_mode() & ttBranchTaken)) {
          core->get_logger()->log_branch_end(core->CIA_read().uint64());
        }
    ...
    // Decode-cache translation function for instruction "bc2"
    inline void _sym69(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> BO, bits<5> BI, bits<14> BD_p)
    {
      sbits<32> BD = ((BD_p.signExtend<32>() << 2) + pc);
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym75(DcBasicBlock *bb,addr_t pc,unsigned count, bits<3> BF, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_30(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_cmpi(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym81(DcBasicBlock *bb,addr_t pc,unsigned count, bits<3> BF, bits<5> RA, bits<16> SI)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "crand".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_33(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"crand",core->instr_disasm_crand,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_crand(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "crand"
    inline void _sym87(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "crandc".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_36(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"crandc",core->instr_disasm_crandc,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_crandc(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "crandc"
    inline void _sym93(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "creqv".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_39(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"creqv",core->instr_disasm_creqv,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_creqv(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "creqv"
    inline void _sym99(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "crnand".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_42(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"crnand",core->instr_disasm_crnand,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_crnand(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "crnand"
    inline void _sym105(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "crnor".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_45(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"crnor",core->instr_disasm_crnor,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_crnor(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "crnor"
    inline void _sym111(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "cror".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_48(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"cror",core->instr_disasm_cror,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_cror(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "cror"
    inline void _sym117(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "crorc"
     bool instr_handler_crorc(bits<5> RT, bits<5> RA, bits<5> RB)
     {
      bool __branch_taken__ = false;
      {
            if ( RT == 0 ) {
                  ...
                  commit_rollback (  ) ;
                  {  NIA_write( nia )  ; __branch_taken__ = true ; } ;
              } else if ( RT == 2 ) {
                  flush_rollback (  ) ;
              }
      }
    ...
    // Decode-cache handler for instruction "crorc".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_51(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"crorc",core->instr_disasm_crorc,ifBrUncond);
        }
        if (Tracing && (core->trace_mode() & ttBasicBlockEnd)) {
          core->get_logger()->log_basic_block_end(core->CIA_read().uint64());
        }
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_crorc(dci->_args[1], dci->_args[2], dci->_args[3]);
      ExtCondTrace({
        if (Tracing && (core->trace_mode() & ttBranchTaken)) {
          core->get_logger()->log_branch_end(core->CIA_read().uint64());
        }
    ...
    // Decode-cache translation function for instruction "crorc"
    inline void _sym123(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "crxor".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_54(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"crxor",core->instr_disasm_crxor,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_crxor(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "crxor"
    inline void _sym129(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym135(DcBasicBlock *bb,addr_t pc,unsigned count)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "lmw"
     void _sym666(bits<5> RT, bits<5> RA, bits<16> D)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( b + D ) addr = b + D ;
             bits < 512 > mem_data = mem_read_jit<64>(addr) ;
            if ( RT < 16 ) {
                   GPR_write( to_uint32(0) , (  mem_data.get<32>(0,31)  ) ) ;
                   GPR_write( to_uint32(1) , (  mem_data.get<32>(32,63)  ) ) ;
                   GPR_write( to_uint32(2) , (  mem_data.get<32>(64,95)  ) ) ;
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_60(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym666(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym141(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<16> D)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "lvx"
     void _sym667(bits<5> RT, bits<5> RA, bits<5> RB)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) a = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( a + GPR_read( to_uint32( RB ) ) ) ea = a + GPR_read( to_uint32( RB ) ) ;
             AdlTypeof(ea) orig_ea = ea ;
            ea = ea & ~ 0xfULL ;
             VPR_write( to_uint32( RT ) , (  mem_read_jit<16>(ea,CacheRead,true,orig_ea)  ) ) ;
      }
    }
    
    // Decode-cache handler for instruction "lvx".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_63(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"lvx",core->instr_disasm_lvx,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym667(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "lvx"
    inline void _sym147(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "lwarx"
     void _sym668(bits<5> RT, bits<5> RA, bits<5> RB)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( b + GPR_read( to_uint32( RB ) ) ) addr = b + GPR_read( to_uint32( RB ) ) ;
             GPR_write( to_uint32( RT ) , (  mem_read_jit<4>(addr , MaxCacheAccess + 10 )  ) ) ;
             XPR_write( to_uint32( CTR_read() ) , ( addr ) ) ;
      }
    }
    
    // Decode-cache handler for instruction "lwarx".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_66(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"lwarx",core->instr_disasm_lwarx,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym668(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "lwarx"
    inline void _sym153(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "lwz"
     void _sym669(bits<5> RT, bits<5> RA, bits<16> D)
     {
      {
             AdlTypeof( D .signExtend<32>() ) d = D .signExtend<32>() ;
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_69(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym669(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym159(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<16> D)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "lwzx"
     void _sym670(bits<5> RT, bits<5> RA, bits<5> RB)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( b + GPR_read( to_uint32( RB ) ) ) addr = b + GPR_read( to_uint32( RB ) ) ;
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_72(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym670(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym165(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "mcrxr".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_75(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"mcrxr",core->instr_disasm_mcrxr,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_mcrxr(dci->_args[1]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "mcrxr"
    inline void _sym171(DcBasicBlock *bb,addr_t pc,unsigned count, bits<3> BF)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym177(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<10> SPRN)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "mtmsr".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_81(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"mtmsr",core->instr_disasm_mtmsr,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_mtmsr(dci->_args[1]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "mtmsr"
    inline void _sym183(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym189(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<10> SPRN)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "mulhw".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_87(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"mulhw",core->instr_disasm_mulhw,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_mulhw(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "mulhw"
    inline void _sym195(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym201(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym207(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym213(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<16> UI)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "rfci"
     bool instr_handler_rfci(bits<5> RS, bits<5> RA, bits<5> RB)
     {
      bool __branch_taken__ = false;
      {
             MSR_write(  CSRR1_read()  )  ;
            {   NIA_write(  CSRR0_read()  )   ; __branch_taken__ = true ; } ;
      }
      return __branch_taken__;
    }
    
    // Decode-cache handler for instruction "rfci".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_99(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"rfci",core->instr_disasm_rfci,ifBrUncond);
        }
        if (Tracing && (core->trace_mode() & ttBasicBlockEnd)) {
          core->get_logger()->log_basic_block_end(core->CIA_read().uint64());
        }
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_rfci(dci->_args[1], dci->_args[2], dci->_args[3]);
      ExtCondTrace({
        if (Tracing && (core->trace_mode() & ttBranchTaken)) {
          core->get_logger()->log_branch_end(core->CIA_read().uint64());
        }
    ...
    // Decode-cache translation function for instruction "rfci"
    inline void _sym219(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "rfi"
     bool instr_handler_rfi(bits<5> RS, bits<5> RA, bits<5> RB)
     {
      bool __branch_taken__ = false;
      {
             MSR_write(  SRR1_read()  )  ;
            {   NIA_write(  SRR0_read()  )   ; __branch_taken__ = true ; } ;
      }
      return __branch_taken__;
    }
    
    // Decode-cache handler for instruction "rfi".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_102(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"rfi",core->instr_disasm_rfi,ifBrUncond);
        }
        if (Tracing && (core->trace_mode() & ttBasicBlockEnd)) {
          core->get_logger()->log_basic_block_end(core->CIA_read().uint64());
        }
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_rfi(dci->_args[1], dci->_args[2], dci->_args[3]);
      ExtCondTrace({
        if (Tracing && (core->trace_mode() & ttBranchTaken)) {
          core->get_logger()->log_branch_end(core->CIA_read().uint64());
        }
    ...
    // Decode-cache translation function for instruction "rfi"
    inline void _sym225(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "rlwimi".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_105(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"rlwimi",core->instr_disasm_rlwimi,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_rlwimi(dci->_args[1], dci->_args[2], dci->_args[3], dci->_args[4], dci->_args[5]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "rlwimi"
    inline void _sym231(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> SH, bits<5> MB, bits<5> ME)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    inline void _sym237(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> SH, bits<5> MB, bits<5> ME)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "stmw"
     void _sym671(bits<5> RS, bits<5> RA, bits<16> D)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( b + D ) addr = b + D ;
            ...
            do {
                  mem_write_jit<8>(addr,to_bits<ns_bits>( concat ( GPR_read( to_uint32(r) ) , GPR_read( to_uint32( r + 1 ) ) ) )) ;
                  r = r + 2 ;
                  addr = addr + 8 ;
              } while ( r <= 30 ) ;
            if ( r == 31 ) {
                  mem_write_jit<4>(addr,to_bits<ns_bits>( GPR_read( to_uint32(31) ) )) ;
              }
      }
    }
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_111(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym671(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym243(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<16> D)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "stvx"
     void _sym672(bits<5> RS, bits<5> RA, bits<5> RB)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) a = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( a + GPR_read( to_uint32( RB ) ) ) ea = a + GPR_read( to_uint32( RB ) ) ;
             AdlTypeof(ea) orig_ea = ea ;
            ea = ea & ~ 0xfULL ;
            mem_write_jit<16>(ea,to_bits<ns_bits>( VPR_read( to_uint32( RS ) ) ),CacheWrite,true,orig_ea) ;
      }
    }
    
    // Decode-cache handler for instruction "stvx".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_114(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"stvx",core->instr_disasm_stvx,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym672(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "stvx"
    inline void _sym249(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "stw"
     void _sym673(bits<5> RS, bits<5> RA, bits<16> D)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( D .signExtend<32>() ) d = D .signExtend<32>() ;
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_117(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym673(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym255(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<16> D)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "stwcx."
     void _sym674(bits<5> RS, bits<5> RA, bits<5> RB)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( b + GPR_read( to_uint32( RB ) ) ) addr = b + GPR_read( to_uint32( RB ) ) ;
            mem_write_jit<4>(addr,to_bits<ns_bits>( GPR_read( to_uint32( RS ) ) ) , MaxCacheAccess + 11 ) ;
      }
    }
    
    // Decode-cache handler for instruction "stwcx.".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_120(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"stwcx.",core->instr_disasm_stwcx_,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym674(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "stwcx."
    inline void _sym261(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "stwu"
     void _sym675(bits<5> RS, bits<5> RA, bits<16> D)
     {
      {
             AdlTypeof( D .signExtend<32>() ) d = D .signExtend<32>() ;
             AdlTypeof( GPR_read( to_uint32( RA ) ) + d ) addr = GPR_read( to_uint32( RA ) ) + d ;
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_123(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym675(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym267(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<16> D)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Instruction "stwx"
     void _sym676(bits<5> RS, bits<5> RA, bits<5> RB)
     {
      {
             AdlTypeof( ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ) b = ( RA == 0 ) ? 0 : GPR_read( to_uint32( RA ) ) ;
             AdlTypeof( b + GPR_read( to_uint32( RB ) ) ) addr = b + GPR_read( to_uint32( RB ) ) ;
    ...
    static DcDecodeReturn __fastcall dc_decode_cache_instr_126(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      core->NIA_write_simple(core->CIA_read() + 4);
      core->_sym676(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    ...
    inline void _sym273(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RS, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "tw".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_129(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"tw",core->instr_disasm_tw,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_tw(dci->_args[1], dci->_args[2]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "tw"
    inline void _sym279(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "twi".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_132(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ...
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"twi",core->instr_disasm_twi,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_twi(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "twi"
    inline void _sym285(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> TO, bits<5> RA, bits<16> SI)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
    ...
    // Decode-cache handler for instruction "vaddubm".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_135(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ++(core->_instr_count);
      core->CIA_write(core->NIA_read());
      core->pre_fetch_handler();
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"vaddubm",core->instr_disasm_vaddubm,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_vaddubm(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "vaddubm"
    inline void _sym291(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
      di->set_func(dc_decode_cache_instr_135);
      di->set_arg(0,__sym27[0]);
      di->set_arg(1,RT);
      di->set_arg(2,RA);
      di->set_arg(3,RB);
    ...
    // Decode-cache handler for instruction "vadduhm".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_138(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ++(core->_instr_count);
      core->CIA_write(core->NIA_read());
      core->pre_fetch_handler();
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"vadduhm",core->instr_disasm_vadduhm,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_vadduhm(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "vadduhm"
    inline void _sym297(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
      di->set_func(dc_decode_cache_instr_138);
      di->set_arg(0,__sym27[0]);
      di->set_arg(1,RT);
      di->set_arg(2,RA);
      di->set_arg(3,RB);
    ...
    // Decode-cache handler for instruction "vadduwm".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_141(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ++(core->_instr_count);
      core->CIA_write(core->NIA_read());
      core->pre_fetch_handler();
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"vadduwm",core->instr_disasm_vadduwm,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_vadduwm(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "vadduwm"
    inline void _sym303(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
      di->set_func(dc_decode_cache_instr_141);
      di->set_arg(0,__sym27[0]);
      di->set_arg(1,RT);
      di->set_arg(2,RA);
      di->set_arg(3,RB);
    ...
    // Decode-cache handler for instruction "xor".
    static DcDecodeReturn __fastcall dc_decode_cache_instr_144(adliss::P*core,DcDecodeItem *dci) FASTCALL
    {
      ++(core->_instr_count);
      core->CIA_write(core->NIA_read());
      core->pre_fetch_handler();
      ExtCondTrace({
        ...
        if (Tracing && (core->trace_mode() & ttInstr)) {
          core->get_logger()->log_instr(dci->_args,8,"xor",core->instr_disasm_xor,0);
        }
      })
      core->NIA_write_simple(core->CIA_read() + 4);
      core->instr_handler_xor(dci->_args[1], dci->_args[2], dci->_args[3]);
      core->post_exec_outer_handler();
      core->_dc_decode_cache_next = &dci[1];
      return (DcVoidFunc)dci[1].func();
    }
    
    // Decode-cache translation function for instruction "xor"
    inline void _sym309(DcBasicBlock *bb,addr_t pc,unsigned count, bits<5> RT, bits<5> RA, bits<5> RB)
    {
      adliss::P::DcDecodeItem *di = _dc_decode_cache.cur();
      di->set_func(dc_decode_cache_instr_144);
      di->set_arg(0,__sym27[0]);
      di->set_arg(1,RT);
      di->set_arg(2,RA);
      di->set_arg(3,RB);
    ...
    void pre_fetch_handler()  ATTRIBUTE_USED {
      _clock_cycle_tally += 1.2;
      if (_clock_cycle_tally >= 1) {
        unsigned n = (unsigned)_clock_cycle_tally;
    ...
    void post_exec_outer_handler()  ATTRIBUTE_USED {
      ...
      if (_clock_cycle_tally >= 1) {
        double n;
        _clock_cycle_tally = modf(_clock_cycle_tally,&n);
      }
    }
    ...
    addr_t dc_translate_instr(addr_t &pc,DcBasicBlock *bb,unsigned count,bool extra_fetch)
    {
      switch (__sym27[0] & 0xfc000000) {
        ...
        case 0xc000000: {
          // Instruction:  "twi"
          _sym285(bb,pc,count, get_TO(__sym27[0]), get_RA(__sym27[0]), get_SI(__sym27[0]));
          pc += 4;
        }
        break;
        case 0x10000000: {
          switch (__sym27[0] & 0x7fe) {
            case 0x0: {
              // Instruction:  "vaddubm"
              _sym291(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x40: {
              // Instruction:  "vadduhm"
              _sym297(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x80: {
              // Instruction:  "vadduwm"
              _sym303(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            default:
        ...
        case 0x2c000000: {
          // Instruction:  "cmpi"
          _sym81(bb,pc,count, get_BF(__sym27[0]), get_RA(__sym27[0]), get_SI(__sym27[0]));
          pc += 4;
        }
        break;
        case 0x34000000: {
        ...
        case 0x40000000: {
          switch (__sym27[0] & 0x3) {
            case 0x0: {
              // Instruction:  "bc"
              _sym63(bb,pc,count, get_BO(__sym27[0]), get_BI(__sym27[0]), get_BD(__sym27[0]));
              pc += 4;
              return false;
            }
            break;
        ...
        case 0x44000000: {
          switch (__sym27[0] & 0x3) {
            case 0x0: {
              // Instruction:  "bc2"
              _sym69(bb,pc,count, get_BO(__sym27[0]), get_BI(__sym27[0]), get_BD(__sym27[0]));
              pc += 4;
              return false;
            }
            break;
        ...
        case 0x48000000: {
          switch (__sym27[0] & 0x3) {
            case 0x0: {
              // Instruction:  "b"
              _sym57(bb,pc,count, get_LI(__sym27[0]));
              pc += 4;
              return false;
            }
            break;
        ...
        case 0x4c000000: {
          switch (__sym27[0] & 0x7fe) {
            case 0x42: {
              // Instruction:  "crnor"
              _sym111(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x64: {
              // Instruction:  "rfi"
              _sym225(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
              return false;
            }
            break;
            case 0x66: {
              // Instruction:  "rfci"
              _sym219(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
              return false;
            }
            break;
            case 0x102: {
              // Instruction:  "crandc"
              _sym93(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x182: {
              // Instruction:  "crxor"
              _sym129(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x1c2: {
              // Instruction:  "crnand"
              _sym105(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x202: {
              // Instruction:  "crand"
              _sym87(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x242: {
              // Instruction:  "creqv"
              _sym99(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x342: {
              // Instruction:  "crorc"
              _sym123(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
              return false;
            }
            break;
            case 0x382: {
              // Instruction:  "cror"
              _sym117(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            default:
        ...
        case 0x50000000: {
          // Instruction:  "rlwimi"
          _sym231(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_SH(__sym27[0]), get_MB(__sym27[0]), get_ME(__sym27[0]));
          pc += 4;
        }
        break;
        case 0x54000000: {
          // Instruction:  "rlwinm"
          _sym237(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_SH(__sym27[0]), get_MB(__sym27[0]), get_ME(__sym27[0]));
          pc += 4;
        }
        break;
        case 0x60000000: {
          // Instruction:  "ori"
          _sym213(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_UI(__sym27[0]));
          pc += 4;
        }
        break;
        case 0x7c000000: {
          switch (__sym27[0] & 0x7fe) {
            case 0x0: {
              // Instruction:  "cmp"
              _sym75(bb,pc,count, get_BF(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x8: {
              // Instruction:  "tw"
              _sym279(bb,pc,count, get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x28: {
              // Instruction:  "lwarx"
              _sym153(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x2e: {
              // Instruction:  "lwzx"
              _sym165(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x96: {
              // Instruction:  "mulhw"
              _sym195(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0xce: {
              // Instruction:  "lvx"
              _sym147(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x124: {
              // Instruction:  "mtmsr"
              _sym183(bb,pc,count, get_RS(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x12c: {
              // Instruction:  "stwcx."
              _sym261(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x12e: {
              // Instruction:  "stwx"
              _sym273(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x1ce: {
              // Instruction:  "stvx"
              _sym249(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x1d6: {
              // Instruction:  "mullw"
              _sym201(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x214: {
            ...
            case 0x278: {
              // Instruction:  "xor"
              _sym309(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x2a6: {
              // Instruction:  "mfspr"
              _sym177(bb,pc,count, get_RT(__sym27[0]), get_SPRN(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x378: {
              // Instruction:  "or"
              _sym207(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_RB(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x3a6: {
              // Instruction:  "mtspr"
              _sym189(bb,pc,count, get_RS(__sym27[0]), get_SPRN(__sym27[0]));
              pc += 4;
            }
            break;
            case 0x400: {
              // Instruction:  "mcrxr"
              _sym171(bb,pc,count, get_BF(__sym27[0]));
              pc += 4;
            }
            break;
            default:
        ...
        case 0x80000000: {
          // Instruction:  "lwz"
          _sym159(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_D(__sym27[0]));
          pc += 4;
        }
        break;
        case 0x90000000: {
          // Instruction:  "stw"
          _sym255(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_D(__sym27[0]));
          pc += 4;
        }
        break;
        case 0x94000000: {
          // Instruction:  "stwu"
          _sym267(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_D(__sym27[0]));
          pc += 4;
        }
        break;
        case 0xb8000000: {
          // Instruction:  "lmw"
          _sym141(bb,pc,count, get_RT(__sym27[0]), get_RA(__sym27[0]), get_D(__sym27[0]));
          pc += 4;
        }
        break;
        case 0xbc000000: {
          // Instruction:  "stmw"
          _sym243(bb,pc,count, get_RS(__sym27[0]), get_RA(__sym27[0]), get_D(__sym27[0]));
          pc += 4;
        }
        break;
        default:
  ...
  #ifndef ADL_HEADER
  static bool dummy ATTRIBUTE_UNUSED = log_usage("mod3","2018.2.28",0,"model","make-iss");
  #endif
