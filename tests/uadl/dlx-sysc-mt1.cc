...
#  define ADL_HEADER
#endif
#include "dlx-sysc-mt1-iss.cc"
#define _OSM_Thread_IMPLEMENTATION_H
#include "dlx-sysc-mt1.h"
...
namespace uadl {
    ...
    void write(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger) {
      ...
      for (int i = 1; i != 33; ++i) {
        if (ic.op(i).isTrg()) {
          ...
          if (logger)logger->logOpAction("write",static_cast<OSM_Thread&>(uarch).GPR.name(),ic.op(i)._index,ic._instr->slot(i).uint64());
        }
      }
    }
  ...
  struct mtmsr_mtmsr_Trg {
    ...
    void write(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),1);
      if (logger)logger->logOpAction("write",static_cast<OSM_Thread&>(uarch).MSR.name(),-1,ic._instr->slot(1).uint64());
    }
  ...
  struct addic__sfx_Src {
    ...
    void read(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger,T0 &t0,T1 &t1) {
      ...
      {
        read(uarch,ic,logger,1);
        if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).XER.id(static_cast<OSM_Thread&>(uarch),0)).first != 0) {
          t0.read_partial(uarch,ic,logger,*this,1);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name(),1);
        }
        if (t1.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).XER.id(static_cast<OSM_Thread&>(uarch),0)).first != 0) {
  ...
  struct addic_sfx_Trg {
    ...
    void write(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("write",static_cast<OSM_Thread&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
      ic._instr->commit(uarch.get_iss(),2);
      if (logger)logger->logOpAction("write",static_cast<OSM_Thread&>(uarch).XER.name(),-1,ic._instr->slot(2).uint64());
    }
  ...
  struct srw_sfx_Src {
    ...
    void read(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger,T0 &t0,T1 &t1) {
      {
        read(uarch,ic,logger,0);
        if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).GPR.id(static_cast<OSM_Thread&>(uarch),ic.op(1)._index)).first != 0) {
          t0.read_partial(uarch,ic,logger,*this,0);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name(),0);
        }
        if (t1.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).GPR.id(static_cast<OSM_Thread&>(uarch),ic.op(1)._index)).first != 0) {
  ...
  struct stmw_stmw_Src {
    ...
    bool read(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger) {
      ...
      for (int i = 1; i != 33; ++i) {
        if (ic.op(i).isSrc()) {
          ...
          if (logger)logger->logOpAction("read",static_cast<OSM_Thread&>(uarch).GPR.name(),ic.op(i)._index,ic._instr->slot(i).uint64());
        }
      }
      return true;
    }
    ...
    void log_read(UadlArch &uarch,Inst_PMT &ic,Logger *logger,const string &msg) {
      ...
      for (int i = 1; i != 33; ++i) {
        if (ic.op(i).isSrc()) {
          logger->logOpAction(msg,static_cast<OSM_Thread&>(uarch).GPR.name(),ic.op(i)._index,ic._instr->slot(i).uint64());
        }
      }
    }
    ...
    void read(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger,T0 &t0,T1 &t1) {
      {
        read(uarch,ic,logger,0);
        if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).GPR.id(static_cast<OSM_Thread&>(uarch),ic.op(0)._index)).first != 0) {
          t0.read_partial(uarch,ic,logger,*this,0);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name(),0);
        }
        if (t1.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).GPR.id(static_cast<OSM_Thread&>(uarch),ic.op(0)._index)).first != 0) {
          t1.read_partial(uarch,ic,logger,*this,0);
          if (logger) log_read(uarch,ic,logger,"read from "+t1.name(),0);
        }
      }
      for (int i = 1; i != 33; ++i) {
        if  (ic.op(i).isSrc()) {
          ic._instr->read_operand(uarch.get_iss(),i);
          if (logger)logger->logOpAction("read",static_cast<OSM_Thread&>(uarch).GPR.name(),ic.op(i)._index,ic._instr->slot(i).uint64());
          if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).GPR.id(static_cast<OSM_Thread&>(uarch),ic.op(i)._index)).first != 0) {
            t0.read_partial(uarch,ic,logger,*this,1,i);
            if (logger) logger->logOpAction("read from "+t0.name(),static_cast<OSM_Thread&>(uarch).GPR.name(),ic.op(i)._index,ic._instr->slot(i).uint64());
          }
          if (t1.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).GPR.id(static_cast<OSM_Thread&>(uarch),ic.op(i)._index)).first != 0) {
            t1.read_partial(uarch,ic,logger,*this,1,i);
            if (logger) logger->logOpAction("read from "+t1.name(),static_cast<OSM_Thread&>(uarch).GPR.name(),ic.op(i)._index,ic._instr->slot(i).uint64());
          }
        }
      }
    }
  ...
  struct stmw_stmw_SrcCR {
    ...
    void read(OSM_Thread &uarch,Inst_PMT &ic,Logger *logger,T0 &t0,T1 &t1) {
      {
        read(uarch,ic,logger);
        if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).CR.id(static_cast<OSM_Thread&>(uarch),0)).first != 0) {
          t0.read_partial(uarch,ic,logger,*this);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name());
        }
        if (t1.can_read_partial(uarch,ic,logger,static_cast<OSM_Thread&>(uarch).CR.id(static_cast<OSM_Thread&>(uarch),0)).first != 0) {
  ...
  void OSM_Thread::getMemoryData(CacheAccess type, addr_t addr, unsigned size, byte_t *data)
  {
    if (type == adl::CacheLogRead) {
      ...
      if (L1d_obj.getMemoryData(type, addr, size, data)) {
        ...
        if (Inst_PMT *instr = mMM[0]) {
          getMemoryData(data,instr->mem(),addr,size);
        }
        if (Inst_PMT *instr = mMM[1]) {
          getMemoryData(data,instr->mem(),addr,size);
        }
        if (Inst_PMT *instr = mEX[0]) {
          getMemoryData(data,instr->mem(),addr,size);
        }
        if (Inst_PMT *instr = mEX[1]) {
          getMemoryData(data,instr->mem(),addr,size);
        }
  ...
  void OSM_Thread::setIssMemHandler(adl::MemHandler *memhandler)
  {
    _iss->setMemHandler(memhandler);
  }
  
  void OSM_Thread::setExtMemory(UadlMemoryIf &mem)
  {
    Mem_obj.setMemory(mem);
    Mem_obj.addPort("D",0, 2, 2, 0, 0, 2,1);
    Mem_obj.addPort("I",0, 2, 2, 0, 0, 2,1);
    Mem_obj.setMemParms(
    1,
    1,
    1,
  ...
  bool OSM_Thread::hasExtMemSet() const
  {
  ...
  OSM_Thread::OSM_Thread(unsigned &id, unsigned &thread_id, std::ostream &out, ttime_t startTime, UadlMemoryIf *memory, PipelineStage<2,0,OSM_Thread,Inst_PMT,1> &p_mEX, PipelineStage<2,0,OSM_Thread,Inst_PMT,1> &p_mID, PipelineStage<2,0,OSM_Thread,Inst_PMT,1> &p_mMM, PipelineStage<2,0,OSM_Thread,Inst_PMT,1> &p_mWB, MemoryDelegate &p_Mem_obj, L1d_cache_t &p_L1d_obj, L1i_cache_t &p_L1i_obj, RegFileResource<uint32_t,2> &p_BARs, RegResource<uint32_t> &p_BootAddr, RegResource<uint32_t> &p_CBTEST2, RegResource<uint32_t> &p_CCR, RegFileResource<uint32_t,2> &p_FOOs, RegResource<uint32_t> &p_HID0, RegResource<uint32_t> &p_LoadStoreCount, RegResource<uint32_t> &p_LoadStoreIndex, RegFileResource<uint32_t,2> &p_MSRs, RegFileResource<uint32_t,2> &p_NIAs, RegFileResource<uint32_t,2> &p_TCR, RegResource<uint32_t> &p_TEN, Semaphore &p_all_stall, Flag &p_hid_stall, ForwardingPath<Inst_PMT,0,0,6,32> &p_FwdEX, ForwardingPath<Inst_PMT,0,0,6,32> &p_FwdMM, bool &p__stall, OSM_PMT &top_p, ISS_PMT_Thread *iss) : 
  ...
  void OSM_PMT::setIssMemHandler(adl::MemHandler *memhandler)
  {
    _iss->setMemHandler(memhandler);
  }
  
  void OSM_PMT::setExtMemory(UadlMemoryIf &mem)
  {
    t0.setExtMemory(mem);
    t1.setExtMemory(mem);
  }
  
  bool OSM_PMT::hasExtMemSet() const
  {
    return t0.hasExtMemSet();
  }
  ...
  void OSM_PMT::start_of_simulation()
  {
    preRun();
  }
  
  SC_HAS_PROCESS(OSM_PMT);
  
  OSM_PMT::~OSM_PMT() {
    delete _iss;
  }
  
  OSM_PMT::OSM_PMT(sc_module_name name, unsigned &id, unsigned &thread_id, std::ostream &out, ttime_t startTime, UadlMemoryIf *memory) : 
  sc_module(name), 
  ...
  _stage_loggers(8), 
  top(*this), 
  t0(id, thread_id, out, startTime, memory, mEX, mID, mMM, mWB, Mem_obj, L1d_obj, L1i_obj, BARs, BootAddr, CBTEST2, CCR, FOOs, HID0, LoadStoreCount, LoadStoreIndex, MSRs, NIAs, TCR, TEN, all_stall, hid_stall, FwdEX, FwdMM, _stall, *this,static_cast<ISS_PMT_Thread*>(_iss->t0)), 
  t1(id, thread_id, out, startTime, memory, mEX, mID, mMM, mWB, Mem_obj, L1d_obj, L1i_obj, BARs, BootAddr, CBTEST2, CCR, FOOs, HID0, LoadStoreCount, LoadStoreIndex, MSRs, NIAs, TCR, TEN, all_stall, hid_stall, FwdEX, FwdMM, _stall, *this,static_cast<ISS_PMT_Thread*>(_iss->t1))
  {
    L1d_obj.set_level(0);
    L1i_obj.set_level(0);
