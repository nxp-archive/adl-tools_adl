...
#  define ADL_HEADER
#endif
#include "dlx-bad-br-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-bad-br.h"
...
namespace uadl {
  ...
  struct e_lmvsprw_lmv_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).CTR.name(),-1);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).LR.name(),-1);
  ...
  void OSM_P::signalException(Inst_P *instr) {
    if (IF.logger()) IF.logger()->logMessage("commiting exception");
