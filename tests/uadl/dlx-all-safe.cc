...
#  define ADL_HEADER
#endif
#include "dlx-all-safe-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-all-safe.h"
...
namespace uadl {
  ...
  struct Inst_sfx : public Inst_P {
    ...
    // State action S_ID
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read_fp(uarch,*this,logger, uarch.FwdEX , uarch.FwdMM ) && Trg.can_write(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) && uarch.L1d.can_lock(uarch,*this,logger) ) {
            Src.read(uarch,*this,logger, uarch.FwdEX , uarch.FwdMM ) ;
            Trg.allocate(uarch,*this,logger) ;
            uarch.L1d.allocate_lock(uarch,*this,logger) ;
            return change_state_S_ID__S_EX(uarch,logger,position) ;
      }
      return false;
    }
    ...
    bool action_S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mWB.has_space(uarch,*this,logger) ) {
            uarch.FwdMM.write(uarch,*this,logger, Trg ) ;
            uarch.L1d.deallocate_lock(uarch,*this,logger) ;
            return change_state_S_MM__S_WB(uarch,logger,position) ;
      }
      return false;
    }
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym921[0],&_sym921[13]),
      0  // with-data
    ),
    _uarch(uarch)
  {
  ...
  mMEM(mMEM_obj), 
  _log_mem(true), 
  L1d_obj(*this,mMEM_obj), 
  L1d(L1d_obj), 
  L1i_obj(*this,mMEM_obj), 
  L1i(L1i_obj), 
  CIA("CIA"), 
  CR("CR"), 
  CTR("CTR"), 
  ...
  FwdEX("FwdEX"), 
  FwdMM("FwdMM"), 
  Fetcher(*this,getPath(_iss,".")+"Fetcher",2,L1i_obj,0,_fetchSizes,&mMEM_obj), 
  _cur_instr(0), 
  _last_instr(0), 
  _speculative(false), 
  _branch_target(0), 
  ...
  bool OSM_P::active() const
  {
    ...
    return (_iss->activeCount() || instr_alloc_count() || mMEM_obj.isActive() || L1d_obj.isActive() || L1i_obj.isActive());
  }
