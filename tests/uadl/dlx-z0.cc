...
namespace uadl {
  ...
  struct bcl_branch___other__ {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger && ic._instr->is_dirty(4))logger->logOpAction("write",static_cast<OSM_P&>(uarch).NIA.name(),-1,ic._instr->slot(4).uint64());
    }
  ...
  struct bcctr_branch___other__ {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),2);
      if (logger && ic._instr->is_dirty(2))logger->logOpAction("write",static_cast<OSM_P&>(uarch).NIA.name(),-1,ic._instr->slot(2).uint64());
    }
  ...
  struct bclrl_branch___other__ {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).LR.name(),-1,ic._instr->slot(3).uint64());
      return true;
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      ...
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).CR.name(),-1,ic._instr->slot(1).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).CTR.name(),-1,ic._instr->slot(2).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).LR.name(),-1,ic._instr->slot(3).uint64());
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        ...
        case 2: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).CTR.name(),-1,ic._instr->slot(2).uint64());
        break;
        case 3: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).LR.name(),-1,ic._instr->slot(3).uint64());
        break;
        case 4: break;
      }
    }
  ...
  struct Inst_branch : public Inst_P {
    ...
    bool change_state_S_EX__S_WB(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EX -> S_WB"); }
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mWB.has_space(uarch,*this,logger) ) {
            return change_state_S_EX__S_WB(uarch,logger,position) ;
      }
      return false;
    }
  ...
  struct rfi_jump___other__ {
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        case 0: break;
        case 1: break;
        case 2: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).SRR0.name(),-1,ic._instr->slot(2).uint64());
        break;
        case 3: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).SRR1.name(),-1,ic._instr->slot(3).uint64());
        break;
      }
    }
  ...
  struct rfci_jump___other__ {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ic._instr->read_operand(uarch.get_iss(),0);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).CSRR0.name(),-1,ic._instr->slot(0).uint64());
      ic._instr->read_operand(uarch.get_iss(),1);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).CSRR1.name(),-1,ic._instr->slot(1).uint64());
      return true;
    }
    
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).CSRR0.name(),-1,ic._instr->slot(0).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).CSRR1.name(),-1,ic._instr->slot(1).uint64());
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        case 0: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).CSRR0.name(),-1,ic._instr->slot(0).uint64());
        break;
        case 1: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).CSRR1.name(),-1,ic._instr->slot(1).uint64());
        break;
        case 2: break;
        case 3: break;
        case 4: break;
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),2);
      if (logger && ic._instr->is_dirty(2))logger->logOpAction("write",static_cast<OSM_P&>(uarch).ESR.name(),-1,ic._instr->slot(2).uint64());
      ic._instr->commit(uarch.get_iss(),3);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(3).uint64());
      ic._instr->commit(uarch.get_iss(),4);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).NIA.name(),-1,ic._instr->slot(4).uint64());
    }
  ...
  struct lmw_lmw_Src {
    ...
    template <typename T0>
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(0)._index,"read"))) return false;
      }
      return true;
    }
    
    template <typename T0>
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        RegId id = static_cast<OSM_P&>(uarch).GPR.id(static_cast<OSM_P&>(uarch),ic.op(0)._index);
        tag_t tag = (static_cast<OSM_P&>(uarch).GPR.last_allocator( static_cast<OSM_P&>(uarch),ic.op(0)._index));
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        for (int i = 1; i != 33; ++i) {
          if  (ic.op(i).isSrc()) {
            ...
            if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(i)._index,"read"))) return false;
          }
        }
      }
      return true;
  ...
  struct Inst_lmw : public Inst_P {
    ...
    bool change_state_S_EXp__S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EXp -> S_EX"); }
      _action = (StateFunc)action_S_EX_p;
      _flush = (FlushFunc)flush_S_EX_p;
      return action_S_EX(uarch,logger,position);
    }
    
    bool change_state_S_EXp__S_WB(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EXp -> S_WB"); }
      _action = (StateFunc)action_S_WB_p;
      _flush = (FlushFunc)flush_S_WB_p;
      uarch.mEX.remove_instr();
      uarch.mWB.add_instr(this);
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( uarch.mMEM.can_read(uarch,*this,logger) ) {
              uarch.exec_and_read(*this,logger, uarch.mMEM , 2 ) ;
    ...
    bool action_S_EXp(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( exec_done (  ) && uarch.mWB.has_space(uarch,*this,logger) ) {
              uarch.Fwd.write(uarch,*this,logger, Trg ) ;
              return change_state_S_EXp__S_WB(uarch,logger,position) ;
        } else if ( uarch.mMEM.can_request_read(uarch,*this,logger) ) {
              uarch.mMEM.send_read_request(uarch,*this,logger, 8 ) ;
              next_req_mem (  ) ;
              return change_state_S_EXp__S_EX(uarch,logger,position) ;
        }}
      catch (adliss::P::Exception) {
        if (logger) logger->logMessage("exception occurred");
        uarch.cancel_all_mem(*this);
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( Src.can_read_fp(uarch,*this,logger, uarch.Fwd ) && Trg.can_write(uarch,*this,logger) ) {
              Src.read(uarch,*this,logger, uarch.Fwd ) ;
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(1)._index,"read"))) return false;
      }
      return true;
    }
  ...
  struct lbzux_load_Trg {
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(2)._index,"read"))) return false;
      }
      return true;
    }
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.SPR.can_alloc(uarch,ic,logger,ic.op(1)._index,"read"))) return false;
      }
      return true;
    }
  ...
  struct mfspr_mftspr_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),0);
      if (logger && ic._instr->is_dirty(0))logger->logOpAction("write",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(0)._index,ic._instr->slot(0).uint64());
    }
  ...
  struct mtspr_mftspr_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),1);
      if (logger && ic._instr->is_dirty(1))logger->logOpAction("write",static_cast<OSM_P&>(uarch).SPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
    }
  ...
  struct Inst_mftspr : public Inst_P {
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( Src.can_read_fp(uarch,*this,logger, uarch.Fwd ) && Trg.can_write(uarch,*this,logger) && uarch.msync_stall.can_allocate(uarch,*this,logger) && uarch.mEX.empty(uarch,*this,logger) && uarch.mWB.empty(uarch,*this,logger) ) {
              Src.read(uarch,*this,logger, uarch.Fwd ) ;
              Trg.allocate(uarch,*this,logger) ;
              return change_state_S_ID__S_EX(uarch,logger,position) ;
        }}
  ...
  struct add__sfx_Src {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(4).uint64());
      return true;
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      ...
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(4).uint64());
    }
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger,unsigned sub_op) {
      switch (sub_op) {
        ...
        if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(3)._index,ic._instr->slot(3).uint64());
        break;
        case 3: ic._instr->read_operand(uarch.get_iss(),4);
        if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(4).uint64());
        break;
      }
      return true;
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        ...
        case 1: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
        break;
        case 2: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(3)._index,ic._instr->slot(3).uint64());
        break;
        case 3: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(4).uint64());
        break;
      }
    }
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(3)._index,"read"))) return false;
      }
      {
        bits<32> mask = 0;
        {
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.CR.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      {
        RegId id = static_cast<OSM_P&>(uarch).GPR.id(static_cast<OSM_P&>(uarch),ic.op(3)._index);
        tag_t tag = (static_cast<OSM_P&>(uarch).GPR.last_allocator( static_cast<OSM_P&>(uarch),ic.op(3)._index));
  ...
  struct extsb__sfx_Src {
    ...
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      {
        read(uarch,ic,logger,1);
        if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_P&>(uarch).GPR.id(static_cast<OSM_P&>(uarch),ic.op(2)._index)).first != 0) {
          t0.read_partial(uarch,ic,logger,*this,1);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name(),1);
        }
      }
  ...
  struct cmpli_sfx_Src {
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        ...
        case 1: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
        break;
        case 2: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(2).uint64());
        break;
      }
    }
  ...
  struct ba_sfx_Trg {
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.NIA.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      return true;
    }
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),0);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).NIA.name(),-1,ic._instr->slot(0).uint64());
    }
  ...
  struct bca_sfx_Src {
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.CTR.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      return true;
    }
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.CIA.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      {
        RegId id = static_cast<OSM_P&>(uarch).CR.id(static_cast<OSM_P&>(uarch),0);
        tag_t tag = (static_cast<OSM_P&>(uarch).CR.last_allocator( static_cast<OSM_P&>(uarch),0));
  ...
  struct bcla_sfx_Trg {
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.LR.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      {
        RegId id = static_cast<OSM_P&>(uarch).NIA.id(static_cast<OSM_P&>(uarch),0);
        tag_t tag = (static_cast<OSM_P&>(uarch).NIA.last_allocator( static_cast<OSM_P&>(uarch),0));
  ...
  struct lwarx_sfx_Trg {
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.RESRV.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      return true;
    }
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
      ic._instr->commit(uarch.get_iss(),3);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).RESRV.name(),-1,ic._instr->slot(3).uint64());
    }
    ...
    bits<64> value(OSM_P &uarch,Inst_P &ic,Logger *logger) const {
      return ic._instr->slot(2);
    }
  ...
  struct mcrxr_sfx_Src {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ic._instr->read_operand(uarch.get_iss(),1);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(1).uint64());
      return true;
    }
    
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(1).uint64());
    }
  ...
  struct mcrxr_sfx_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).CR.name(),-1,ic._instr->slot(0).uint64());
      ic._instr->commit(uarch.get_iss(),1);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(1).uint64());
    }
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.MSR.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      return true;
    }
  ...
  struct sc_sfx_Src {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      ic._instr->read_operand(uarch.get_iss(),6);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(6)._index,ic._instr->slot(6).uint64());
      return true;
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      ...
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(6)._index,ic._instr->slot(6).uint64());
    }
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger,unsigned sub_op) {
      switch (sub_op) {
        ...
        if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(3)._index,ic._instr->slot(3).uint64());
        break;
        case 4: ic._instr->read_operand(uarch.get_iss(),4);
        if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(4)._index,ic._instr->slot(4).uint64());
        break;
        case 5: ic._instr->read_operand(uarch.get_iss(),5);
        if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(5)._index,ic._instr->slot(5).uint64());
        break;
        case 6: ic._instr->read_operand(uarch.get_iss(),6);
        if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(6)._index,ic._instr->slot(6).uint64());
        break;
      }
      return true;
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        ...
        case 2: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
        break;
        case 3: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(3)._index,ic._instr->slot(3).uint64());
        break;
        case 4: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(4)._index,ic._instr->slot(4).uint64());
        break;
        case 5: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(5)._index,ic._instr->slot(5).uint64());
        break;
        case 6: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(6)._index,ic._instr->slot(6).uint64());
        break;
      }
    }
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(4)._index,"read"))) return false;
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(5)._index,"read"))) return false;
      }
      {
        ...
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || uarch.GPR.can_alloc(uarch,ic,logger,ic.op(6)._index,"read"))) return false;
      }
      return true;
    }
    ...
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      }
      {
        ...
        tag_t tag = (static_cast<OSM_P&>(uarch).GPR.last_allocator( static_cast<OSM_P&>(uarch),ic.op(5)._index));
        if (t0.can_read(uarch,ic,logger,id,tag)) {
          t0.read(uarch,ic,logger,*this,5);
      ...
      }
      {
        ...
        tag_t tag = (static_cast<OSM_P&>(uarch).GPR.last_allocator( static_cast<OSM_P&>(uarch),ic.op(6)._index));
        if (t0.can_read(uarch,ic,logger,id,tag)) {
          t0.read(uarch,ic,logger,*this,6);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name(),6);
        }
        else {
          read(uarch,ic,logger,6);
        }
      }
    }
  ...
  struct sc_sfx_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),2);
      if (logger && ic._instr->is_dirty(2))logger->logOpAction("write",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
    }
  ...
  struct std_sfx_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),2);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).dsESR.name(),-1,ic._instr->slot(2).uint64());
    }
  ...
  struct stdux_sfx_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(0)._index,ic._instr->slot(0).uint64());
      ic._instr->commit(uarch.get_iss(),3);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).dsESR.name(),-1,ic._instr->slot(3).uint64());
    }
  ...
  struct stwcx__sfx_Src {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(3)._index,ic._instr->slot(3).uint64());
      ic._instr->read_operand(uarch.get_iss(),4);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).RESRV.name(),-1,ic._instr->slot(4).uint64());
      ic._instr->read_operand(uarch.get_iss(),5);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(5).uint64());
      return true;
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      ...
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(3)._index,ic._instr->slot(3).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).RESRV.name(),-1,ic._instr->slot(4).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(5).uint64());
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        ...
        case 3: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(3)._index,ic._instr->slot(3).uint64());
        break;
        case 4: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).RESRV.name(),-1,ic._instr->slot(4).uint64());
        break;
        case 5: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).XER.name(),-1,ic._instr->slot(5).uint64());
        break;
      }
    }
  ...
  struct stwcx__sfx_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).CR.name(),-1,ic._instr->slot(0).uint64());
      ic._instr->commit(uarch.get_iss(),4);
      if (logger && ic._instr->is_dirty(4))logger->logOpAction("write",static_cast<OSM_P&>(uarch).RESRV.name(),-1,ic._instr->slot(4).uint64());
      ic._instr->commit(uarch.get_iss(),6);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).dsESR.name(),-1,ic._instr->slot(6).uint64());
    }
  ...
  struct tw_sfx_Src {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ic._instr->read_operand(uarch.get_iss(),0);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).DBCR0.name(),-1,ic._instr->slot(0).uint64());
      ic._instr->read_operand(uarch.get_iss(),1);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
      ic._instr->read_operand(uarch.get_iss(),2);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
      ic._instr->read_operand(uarch.get_iss(),3);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(3).uint64());
      return true;
    }
    
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).DBCR0.name(),-1,ic._instr->slot(0).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(3).uint64());
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        case 0: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).DBCR0.name(),-1,ic._instr->slot(0).uint64());
        break;
        case 1: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
        break;
        case 2: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(2)._index,ic._instr->slot(2).uint64());
        break;
        case 3: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(3).uint64());
        break;
      }
    }
    ...
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        read(uarch,ic,logger,0);
        if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_P&>(uarch).DBCR0.id(static_cast<OSM_P&>(uarch),0)).first != 0) {
          t0.read_partial(uarch,ic,logger,*this,0);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name(),0);
        }
      }
  ...
  struct twi_sfx_Src {
    ...
    bool read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
      ic._instr->read_operand(uarch.get_iss(),2);
      if (logger)logger->logOpAction("read",static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(2).uint64());
      return true;
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg) {
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).DBCR0.name(),-1,ic._instr->slot(0).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
      logger->logOpAction(msg,static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(2).uint64());
    }
    ...
    void log_read(UadlArch &uarch,Inst_P &ic,Logger *logger,const string &msg,unsigned sub_op) {
      switch (sub_op) {
        ...
        case 1: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).GPR.name(),ic.op(1)._index,ic._instr->slot(1).uint64());
        break;
        case 2: logger->logOpAction(msg,static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(2).uint64());
        break;
      }
    }
  ...
  struct wrtee_sfx_Src {
    ...
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      {
        read(uarch,ic,logger);
        if (t0.can_read_partial(uarch,ic,logger,static_cast<OSM_P&>(uarch).GPR.id(static_cast<OSM_P&>(uarch),ic.op(0)._index)).first != 0) {
          t0.read_partial(uarch,ic,logger,*this);
          if (logger) log_read(uarch,ic,logger,"read from "+t0.name());
        }
      }
  ...
  struct wrtee_sfx_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),1);
      if (logger && ic._instr->is_dirty(1))logger->logOpAction("write",static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(1).uint64());
    }
  ...
  struct wrteei_sfx_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),0);
      if (logger && ic._instr->is_dirty(0))logger->logOpAction("write",static_cast<OSM_P&>(uarch).MSR.name(),-1,ic._instr->slot(0).uint64());
    }
  ...
  struct Inst_sfx : public Inst_P {
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( Src.can_read_fp(uarch,*this,logger, uarch.Fwd ) && Trg.can_write(uarch,*this,logger) && uarch.msync_stall.can_allocate(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) ) {
              Src.read(uarch,*this,logger, uarch.Fwd ) ;
              Trg.allocate(uarch,*this,logger) ;
              return change_state_S_ID__S_EX(uarch,logger,position) ;
        }}
  ...
  struct Inst_signedMulCr : public Inst_P {
    ...
    // State action S_ID
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read_fp(uarch,*this,logger, uarch.Fwd ) && Trg.can_write(uarch,*this,logger) && uarch.msync_stall.can_allocate(uarch,*this,logger) && uarch.CR.can_read(uarch,*this,logger, 0xff000000 ) && uarch.mEX.has_space(uarch,*this,logger) ) {
            Src.read(uarch,*this,logger, uarch.Fwd ) ;
            Trg.allocate(uarch,*this,logger) ;
    ...
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0) {
      ...
      for (int i = 1; i != 33; ++i) {
        if  (ic.op(i).isSrc()) {
          ...
          tag_t tag = (static_cast<OSM_P&>(uarch).GPR.last_allocator( static_cast<OSM_P&>(uarch),ic.op(i)._index));
          if (t0.can_read(uarch,ic,logger,id,tag)) {
            t0.read(uarch,ic,logger,*this,1,i);
  ...
  struct stmw_stmw_Trg {
    ...
    void write(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      deallocate(uarch,ic,logger);
      ic._instr->commit(uarch.get_iss(),33);
      if (logger)logger->logOpAction("write",static_cast<OSM_P&>(uarch).dsESR.name(),-1,ic._instr->slot(33).uint64());
    }
  ...
  struct Inst_stmw : public Inst_P {
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( Src.can_read_fp(uarch,*this,logger, uarch.Fwd ) && Trg.can_write(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) ) {
              Src.read(uarch,*this,logger, uarch.Fwd ) ;
              Trg.allocate(uarch,*this,logger) ;
              return change_state_S_ID__S_IDp(uarch,logger,position) ;
        }}
    ...
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( uarch.mEX.has_space(uarch,*this,logger) && uarch.lsamd_stall.can_allocate(uarch,*this,logger) && uarch.msync_stall.can_allocate(uarch,*this,logger) && uarch.mMEM.can_request_write(uarch,*this,logger) ) {
              uarch.exec_mem(*this,logger) ;
              uarch.mMEM.send_write_request(uarch,*this,logger, 8 ) ;
              next_req_mem (  ) ;
              return change_state_S_IDp__S_EX(uarch,logger,position) ;
  ...
  struct Inst_store : public Inst_P {
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      try {
        if ( Src.can_read_fp(uarch,*this,logger, uarch.Fwd ) && uarch.mEX.has_space(uarch,*this,logger) ) {
              Src.read(uarch,*this,logger, uarch.Fwd ) ;
              Trg.allocate(uarch,*this,logger) ;
              return change_state_S_ID__S_IDp(uarch,logger,position) ;
        }}
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lbzu_load_Src, lbzu_load_Trg> *ic = (Inst_load<lbzu_load_Src, lbzu_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lbzux_load_Src, lbzux_load_Trg> *ic = (Inst_load<lbzux_load_Src, lbzux_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lbzx_load_Src, lbzx_load_Trg> *ic = (Inst_load<lbzx_load_Src, lbzx_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lha_load_Src, lha_load_Trg> *ic = (Inst_load<lha_load_Src, lha_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lhax_load_Src, lhax_load_Trg> *ic = (Inst_load<lhax_load_Src, lhax_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lhzu_load_Src, lhzu_load_Trg> *ic = (Inst_load<lhzu_load_Src, lhzu_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lhzux_load_Src, lhzux_load_Trg> *ic = (Inst_load<lhzux_load_Src, lhzux_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lhzx_load_Src, lhzx_load_Trg> *ic = (Inst_load<lhzx_load_Src, lhzx_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_load<lwzux_load_Src, lwzux_load_Trg> *ic = (Inst_load<lwzux_load_Src, lwzux_load_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::loadClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<stb_store_Src, stb_store_Trg> *ic = (Inst_store<stb_store_Src, stb_store_Trg>*)_instr_pool.alloc();
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<stbu_store_Src, stbu_store_Trg> *ic = (Inst_store<stbu_store_Src, stbu_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<stbux_store_Src, stbux_store_Trg> *ic = (Inst_store<stbux_store_Src, stbux_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[0] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<stbx_store_Src, stbx_store_Trg> *ic = (Inst_store<stbx_store_Src, stbx_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[1] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<sth_store_Src, sth_store_Trg> *ic = (Inst_store<sth_store_Src, sth_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[0] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<sthu_store_Src, sthu_store_Trg> *ic = (Inst_store<sthu_store_Src, sthu_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[1] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<sthux_store_Src, sthux_store_Trg> *ic = (Inst_store<sthux_store_Src, sthux_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[0] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<sthx_store_Src, sthx_store_Trg> *ic = (Inst_store<sthx_store_Src, sthx_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[1] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<stw_store_Src, stw_store_Trg> *ic = (Inst_store<stw_store_Src, stw_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[0] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<stwux_store_Src, stwux_store_Trg> *ic = (Inst_store<stwux_store_Src, stwux_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[0] = 0x800000;
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_store<stwx_store_Src, stwx_store_Trg> *ic = (Inst_store<stwx_store_Src, stwx_store_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::storeClass,p);
    ic->_masks[1] = 0x800000;
    return ic;
  }
  ...
  void OSM_P::update_tracing()
  {
    ...
    if (logPipeline()) {
      ...
      _stage_loggers.set(1,mEX.logger(0));
      mID.createLogger(_logclient,tflags);
      _stage_loggers.set(2,mID.logger(0));
    } else {
      mWB.removeLogger();
      mEX.removeLogger();
      mID.removeLogger();
  ...
  void OSM_P::cycle()
  {
    try {
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(1+mEX.pos(0)) : 0);
        if (!mEX.empty()) {
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(2+mID.pos(0)) : 0);
        if (!mID.empty()) {
