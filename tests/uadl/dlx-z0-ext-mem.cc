...
#  define ADL_HEADER
#endif
#include "dlx-z0-ext-mem-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-z0-ext-mem.h"
...
namespace uadl {
  ...
  struct Inst_lmw : public Inst_P {
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMEM.can_read(uarch,*this,logger) ) {
            { uarch.exec_and_read(*this,logger, uarch.mMEM , 2 )  ; if (has_exceptions()) goto HandleException; } ;
  ...
  struct Inst_load : public Inst_P {
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMEM.can_read(uarch,*this,logger) ) {
            { uarch.exec_and_read(*this,logger, uarch.mMEM )  ; if (has_exceptions()) goto HandleException; } ;
            return change_state_S_EX__S_EXp(uarch,logger,position) ;
      }
      return false;
      HandleException: {
  ...
  struct Inst_stmw : public Inst_P {
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMEM.can_write(uarch,*this,logger) ) {
            { uarch.exec_and_write(*this,logger, uarch.mMEM , 2 , 4 )  ; if (has_exceptions()) goto HandleException; } ;
            return change_state_S_EX__S_EXp(uarch,logger,position) ;
      }
      return false;
      HandleException: {
  ...
  struct Inst_store : public Inst_P {
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMEM.can_write(uarch,*this,logger) ) {
            { uarch.exec_and_write(*this,logger, uarch.mMEM )  ; if (has_exceptions()) goto HandleException; } ;
            return change_state_S_EX__S_EXp(uarch,logger,position) ;
      }
      return false;
      HandleException: {
  ...
  {
    mMEM_obj.setMemory(mem);
    mMEM_obj.setMemParms(
    1,
    1,
    1,
    1,
  ...
  OSM_P::OSM_P(unsigned &id, unsigned &thread_id, std::ostream &out, ttime_t startTime, UadlMemoryIf *memory) : 
  ...
  UadlArch *createArch(const string &name, unsigned &id,UadlMemoryIf *memory = NULL,ttime_t startTime = 0)
  {
    ...
    return new OSM_P(id,thread_id,std::cout,startTime,memory);
  }
