...
#  define ADL_HEADER
#endif
#include "dlx2-wide-safe-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx2-wide-safe.h"
...
namespace uadl {
  ...
  struct Inst_lmw : public Inst_P {
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.L1d.can_request_read(uarch,*this,logger) ) {
            ...
            uarch.L1d.send_read_request(uarch,*this,logger, 16 ) ;
            next_req_mem (  ) ;
            next_req_mem (  ) ;
            next_req_mem (  ) ;
            return change_state_S_EX__S_MM(uarch,logger,position) ;
    ...
    bool action_S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.L1d.can_read(uarch,*this,logger) ) {
            uarch.exec_and_read(*this,logger, uarch.L1d , 4 ) ;
  ...
  struct Inst_load : public Inst_P {
    ...
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX.has_space(uarch,*this,logger, uarch.mID.capacity() ) && ! uarch.stalled() && uarch.L1d.can_request_read(uarch,*this,logger) ) {
            {uarch.exec_mem(*this,logger) ; if (has_exceptions()) goto HandleException; } ;
            uarch.L1d.send_read_request(uarch,*this,logger) ;
  ...
  struct addme_sfx_Src {
    ...
    bool can_read(OSM_P &uarch,Inst_P &ic,Logger *logger,unsigned sub_op,const bits<32> &mask = bits<32>()) {
      switch (sub_op) {
        case 0: if (!uarch.GPR.can_alloc(uarch,ic,logger,ic.op(0)._index,"read", (ic._masks[2] & ~mask).uint32())) return false;
          return true;
        case 1: if (!uarch.XER.can_alloc(uarch,ic,logger,"read", (ic._masks[3] & ~mask).uint32())) return false;
          return true;
      }
      assert(0);
      return 0;
  ...
  struct srw_sfx_Src {
    bool can_read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      ...
      if (!uarch.GPR.can_alloc(uarch,ic,logger,ic.op(2)._index,"read", ic._masks[4])) return false;
      return true;
    }
    ...
    bool can_read(OSM_P &uarch,Inst_P &ic,Logger *logger,unsigned sub_op,const bits<32> &mask = bits<32>()) {
      switch (sub_op) {
        case 0: if (!uarch.GPR.can_alloc(uarch,ic,logger,ic.op(1)._index,"read", (ic._masks[3] & ~mask).uint32())) return false;
          return true;
        case 1: if (!uarch.GPR.can_alloc(uarch,ic,logger,ic.op(2)._index,"read", (ic._masks[4] & ~mask).uint32())) return false;
          return true;
      }
      assert(0);
      return 0;
  ...
  struct Inst_stmw : public Inst_P {
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.L1d.can_request_write(uarch,*this,logger) ) {
            {uarch.exec_mem(*this,logger) ; if (has_exceptions()) goto HandleException; } ;
            uarch.L1d.send_write_request(uarch,*this,logger, 16 ) ;
            next_req_mem (  ) ;
            next_req_mem (  ) ;
            next_req_mem (  ) ;
            return change_state_S_EX__S_MM(uarch,logger,position) ;
    ...
    bool action_S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.L1d.can_write(uarch,*this,logger) ) {
            uarch.exec_and_write(*this,logger, uarch.L1d , 4 , 4 ) ;
            return change_state_S_MM__S_MMp(uarch,logger,position) ;
      }
      return false;
    }
  ...
  struct Inst_store : public Inst_P {
    ...
    // State action S_IDp
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX.has_space(uarch,*this,logger) ) if ( uarch.mMM.has_space(uarch,*this,logger, uarch.mID.capacity() ) && uarch.L1d.can_request_write(uarch,*this,logger) ) {
            {uarch.exec_mem(*this,logger) ; if (has_exceptions()) goto HandleException; } ;
            uarch.L1d.send_write_request(uarch,*this,logger) ;
            return change_state_S_IDp__S_EX(uarch,logger,position) ;
      }
  ...
  static CacheConfigMap::value_type _sym1040[] = {
    ...
    CacheConfigMap::value_type("linefill_access_width",CacheConfigItem(128)),
  ...
  static CacheConfigMap::value_type _sym1041[] = {
    CacheConfigMap::value_type("critical_word_first",CacheConfigItem(1)),
    CacheConfigMap::value_type("dynamic_entry_size",CacheConfigItem(1)),
    CacheConfigMap::value_type("invalidate_latency",CacheConfigItem(1)),
    CacheConfigMap::value_type("linefill_access_width",CacheConfigItem(128)),
    CacheConfigMap::value_type("linefill_lock_holdoff_cycle",CacheConfigItem(1)),
    CacheConfigMap::value_type("linefill_read_latency",CacheConfigItem(1)),
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym1041[0],&_sym1041[13]),
      0  // with-data
    ),
    _uarch(uarch)
  {
  ...
  unsigned L1i_cache_t::linefillWidth(addr_t ea,addr_t ra)
  {
    return _uarch._iss->_sym410(ea,ra) / 8;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    throw unimpl_instr_exception("Instruction 'get_instr_dcba' is not implemented in this model.");
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    throw unimpl_instr_exception("Instruction 'get_instr_dcbf' is not implemented in this model.");
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    throw unimpl_instr_exception("Instruction 'get_instr_dcbi' is not implemented in this model.");
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    throw unimpl_instr_exception("Instruction 'get_instr_dcbst' is not implemented in this model.");
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    throw unimpl_instr_exception("Instruction 'get_instr_dcbt' is not implemented in this model.");
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    throw unimpl_instr_exception("Instruction 'get_instr_dcbz' is not implemented in this model.");
  }
