...
uint32_t adliss::PMT::Thread::getMemoryData32(CacheAccess type, addr_t addr)
{
...
namespace uadl {
  ...
  struct Inst_cfx : public Inst_PMT {
    ...
    bool action_S_CX0(OSM_Thread &uarch,Logger *logger,unsigned position) {
      ...
      catch (adliss::PMT::Thread::Exception) {
    ...
    bool action_S_ID(OSM_Thread &uarch,Logger *logger,unsigned position) {
      try {
        if ( Trg.can_write(uarch,*this,logger) && uarch.mWB.has_space(uarch,*this,logger) && uarch.mCX0.has_space(uarch,*this,logger) && uarch. OSM_Thread::cfxIssueCheck ( thisInstr (  ) ) ) {
  ...
  static CacheConfigMap::value_type _sym943[] = {
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym943[0],&_sym943[12]),
      1  // with-data
    ),
    _uarch(uarch)
  {
  ...
  {
    return _uarch._iss->t0->L1d_cache_data_array(set, way) + offset;
  }
  ...
  static CacheConfigMap::value_type _sym944[] = {
    CacheConfigMap::value_type("critical_word_first",CacheConfigItem(1)),
    CacheConfigMap::value_type("invalidate_latency",CacheConfigItem(1)),
    CacheConfigMap::value_type("linefill_access_width",CacheConfigItem(64)),
    CacheConfigMap::value_type("linefill_lock_holdoff_cycle",CacheConfigItem(1)),
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym944[0],&_sym944[12]),
      1  // with-data
    ),
    _uarch(uarch)
  {
  ...
  {
    return _uarch._iss->t0->L1i_cache_data_array(set, way) + offset;
  }
  ...
  //
   ...
   bool OSM_Thread:: cfxIssueCheck ( InstrType & instr ) {
         unsigned tid = getChildId (  ) ;
        if ( ( tid == 0 && ! top . t1 . mID . has_attr ( thisCore (  ) , instr , 0 , 0 , adliss::PMT::Thread::cfx_instr ) ) || ( tid == 1 && ! top . t0 . mID . has_attr ( thisCore (  ) , instr , 0 , 0 , adliss::PMT::Thread::cfx_instr ) ) ) return true ;
        return _iss-> cfxProceed ( tid , 2 ) ;
  }
  ...
  Inst_PMT *OSM_Thread::get_instr_b(void *x)
  {
    ...
    ic->reset(this,_issueId++,_commits,adliss::PMT::Thread::serializingClass,p);
    return ic;
  }
  ...
  Inst_PMT *OSM_Thread::get_instr_mulli(void *x)
  {
    ...
    ic->reset(this,_issueId++,_commits,adliss::PMT::Thread::cfxClass,p);
    return ic;
  }
  ...
  void OSM_Thread::taken_flush_mWB(Inst_PMT&ic,Logger *logger,unsigned position,bool given_addr,addr_t addr)
  {
    ...
    for (int i = position+1; i < 4; ++i) {
      if (Inst_PMT *instr = mWB.remove_instr(i)) {
        if (instr->is_alive()) {
  ...
  void OSM_Thread::taken_flush_mID(Inst_PMT&ic,Logger *logger,unsigned position,bool given_addr,addr_t addr)
  {
    ...
    position = 0;
    for (int i = position+1; i < 4; ++i) {
      if (Inst_PMT *instr = mID.remove_instr(i)) {
        if (instr->is_alive()) {
          instr->flush(*this,logger);
          discard(instr);
          instr->set_dead();
  ...
  OSM_Thread::OSM_Thread(unsigned &id, unsigned &thread_id, std::ostream &out, ttime_t startTime, PipelineStage<1,0,OSM_Thread,Inst_PMT,1> &p_mCX0, PipelineStage<1,0,OSM_Thread,Inst_PMT,1> &p_mCX1, PipelineStage<1,0,OSM_Thread,Inst_PMT,1> &p_mCX2, Mem_mem_t &p_Mem_obj, L1d_cache_t &p_L1d_obj, L1i_cache_t &p_L1i_obj, RegFileResource<uint32_t,2> &p_BARs, RegResource<uint32_t> &p_BootAddr, RegResource<uint32_t> &p_CBTEST2, RegResource<uint32_t> &p_CCR, RegResource<uint32_t> &p_CfxIndex, RegFileResource<uint32_t,2> &p_FOOs, RegResource<uint32_t> &p_HID0, RegFileResource<uint32_t,2> &p_MSRs, RegFileResource<uint32_t,2> &p_NIAs, RegFileResource<uint32_t,2> &p_TCR, RegResource<uint32_t> &p_TEN, ForwardingPath<Inst_PMT,0,1,10,32> &p_FwdCB, bool &p__stall, OSM_PMT &top_p, ISS_PMT_Thread *iss) : 
  ...
  void OSM_PMT::update_tracing()
  {
    ...
    t1.update_tracing();
    if (logPipeline()) {
      t0.mWB.createLogger(_logclient,tflags);
      _stage_loggers.set(0,t0.mWB.logger(0));
      _stage_loggers.set(1,t0.mWB.logger(1));
      _stage_loggers.set(2,t0.mWB.logger(2));
      _stage_loggers.set(3,t0.mWB.logger(3));
      t1.mWB.createLogger(_logclient,tflags);
      _stage_loggers.set(4,t1.mWB.logger(0));
      _stage_loggers.set(5,t1.mWB.logger(1));
      _stage_loggers.set(6,t1.mWB.logger(2));
      _stage_loggers.set(7,t1.mWB.logger(3));
      mCX2.createLogger(_logclient,tflags);
      _stage_loggers.set(8,mCX2.logger(0));
      t0.mBR.createLogger(_logclient,tflags);
      _stage_loggers.set(9,t0.mBR.logger(0));
      t0.mEX0.createLogger(_logclient,tflags);
      _stage_loggers.set(10,t0.mEX0.logger(0));
      _stage_loggers.set(11,t0.mEX0.logger(1));
      t0.mLS1.createLogger(_logclient,tflags);
      _stage_loggers.set(12,t0.mLS1.logger(0));
      t1.mBR.createLogger(_logclient,tflags);
      _stage_loggers.set(13,t1.mBR.logger(0));
      t1.mEX0.createLogger(_logclient,tflags);
      _stage_loggers.set(14,t1.mEX0.logger(0));
      _stage_loggers.set(15,t1.mEX0.logger(1));
      t1.mLS1.createLogger(_logclient,tflags);
      _stage_loggers.set(16,t1.mLS1.logger(0));
      mCX1.createLogger(_logclient,tflags);
      _stage_loggers.set(17,mCX1.logger(0));
      t0.mLS0.createLogger(_logclient,tflags);
      _stage_loggers.set(18,t0.mLS0.logger(0));
      t1.mLS0.createLogger(_logclient,tflags);
      _stage_loggers.set(19,t1.mLS0.logger(0));
      mCX0.createLogger(_logclient,tflags);
      _stage_loggers.set(20,mCX0.logger(0));
      t0.mID.createLogger(_logclient,tflags);
      _stage_loggers.set(21,t0.mID.logger(0));
      _stage_loggers.set(22,t0.mID.logger(1));
      _stage_loggers.set(23,t0.mID.logger(2));
      _stage_loggers.set(24,t0.mID.logger(3));
      t1.mID.createLogger(_logclient,tflags);
      _stage_loggers.set(25,t1.mID.logger(0));
      _stage_loggers.set(26,t1.mID.logger(1));
      _stage_loggers.set(27,t1.mID.logger(2));
      _stage_loggers.set(28,t1.mID.logger(3));
    } else {
      t0.mWB.removeLogger();
      t1.mWB.removeLogger();
      mCX2.removeLogger();
      t0.mBR.removeLogger();
      t0.mEX0.removeLogger();
      t0.mLS1.removeLogger();
      t1.mBR.removeLogger();
      t1.mEX0.removeLogger();
      t1.mLS1.removeLogger();
      mCX1.removeLogger();
      t0.mLS0.removeLogger();
      t1.mLS0.removeLogger();
      mCX0.removeLogger();
      t0.mID.removeLogger();
      t1.mID.removeLogger();
  ...
  void OSM_PMT::report_and_clear(ostream &o,const string &pfx)
  {
    *_out << "time " << dec << currTime() << ":\n";
    ...
    t1.mID.report_and_clear(o,pfx);
    t0.mID.report_and_clear(o,pfx);
    mCX0.report_and_clear(o,pfx);
    t1.mLS0.report_and_clear(o,pfx);
    t0.mLS0.report_and_clear(o,pfx);
    mCX1.report_and_clear(o,pfx);
    t1.mLS1.report_and_clear(o,pfx);
    t1.mEX0.report_and_clear(o,pfx);
    t1.mBR.report_and_clear(o,pfx);
    t0.mLS1.report_and_clear(o,pfx);
    t0.mEX0.report_and_clear(o,pfx);
    t0.mBR.report_and_clear(o,pfx);
    mCX2.report_and_clear(o,pfx);
    t1.mWB.report_and_clear(o,pfx);
    t0.mWB.report_and_clear(o,pfx);
    *_out << '\n';
  ...
  void OSM_PMT::cycle()
  {
    try {
      ...
      _stall = false;
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(0+t0.mWB.pos(0)) : 0);
        Logger *logger1 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(0+t0.mWB.pos(1)) : 0);
        Logger *logger2 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(0+t0.mWB.pos(2)) : 0);
        Logger *logger3 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(0+t0.mWB.pos(3)) : 0);
        if (!t0.mWB.empty() && (t0.mWB.head()->is_in_stage(&t0.mWB))) {
          if (t0.mWB.head()->action(*t0.mWB.head()->_uarch,logger0,0)) {
            if (!t0.mWB.empty() && (t0.mWB.head()->is_in_stage(&t0.mWB))) {
              if (t0.mWB.head()->action(*t0.mWB.head()->_uarch,logger1,1)) {
              } else {
                _stall = true;
              }
            }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(4+t1.mWB.pos(0)) : 0);
        Logger *logger1 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(4+t1.mWB.pos(1)) : 0);
        Logger *logger2 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(4+t1.mWB.pos(2)) : 0);
        Logger *logger3 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(4+t1.mWB.pos(3)) : 0);
        if (!t1.mWB.empty() && (t1.mWB.head()->is_in_stage(&t1.mWB))) {
          if (t1.mWB.head()->action(*t1.mWB.head()->_uarch,logger0,0)) {
            if (!t1.mWB.empty() && (t1.mWB.head()->is_in_stage(&t1.mWB))) {
              if (t1.mWB.head()->action(*t1.mWB.head()->_uarch,logger1,1)) {
              } else {
                _stall = true;
              }
            }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(9+t0.mBR.pos(0)) : 0);
        if (!t0.mBR.empty()) {
          if (t0.mBR.head()->action(*t0.mBR.head()->_uarch,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(10+t0.mEX0.pos(0)) : 0);
        Logger *logger1 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(10+t0.mEX0.pos(1)) : 0);
        unsigned sindex = 0;
        if (t0.mEX0[sindex]) {
          if (!t0.mEX0[sindex]->action(*t0.mEX0[sindex]->_uarch,logger0,sindex)) {
            _stall = true;
            ++sindex;
          }
          if (t0.mEX0[sindex]) {
            if (!t0.mEX0[sindex]->action(*t0.mEX0[sindex]->_uarch,logger1,sindex)) {
              _stall = true;
              ++sindex;
            }
          }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(12+t0.mLS1.pos(0)) : 0);
        if (!t0.mLS1.empty()) {
          if (t0.mLS1.head()->action(*t0.mLS1.head()->_uarch,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(13+t1.mBR.pos(0)) : 0);
        if (!t1.mBR.empty()) {
          if (t1.mBR.head()->action(*t1.mBR.head()->_uarch,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(14+t1.mEX0.pos(0)) : 0);
        Logger *logger1 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(14+t1.mEX0.pos(1)) : 0);
        unsigned sindex = 0;
        if (t1.mEX0[sindex]) {
          if (!t1.mEX0[sindex]->action(*t1.mEX0[sindex]->_uarch,logger0,sindex)) {
            _stall = true;
            ++sindex;
          }
          if (t1.mEX0[sindex]) {
            if (!t1.mEX0[sindex]->action(*t1.mEX0[sindex]->_uarch,logger1,sindex)) {
              _stall = true;
              ++sindex;
            }
          }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(16+t1.mLS1.pos(0)) : 0);
        if (!t1.mLS1.empty()) {
          if (t1.mLS1.head()->action(*t1.mLS1.head()->_uarch,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(17+mCX1.pos(0)) : 0);
        if (!mCX1.empty()) {
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(18+t0.mLS0.pos(0)) : 0);
        if (!t0.mLS0.empty()) {
          if (t0.mLS0.head()->action(*t0.mLS0.head()->_uarch,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(19+t1.mLS0.pos(0)) : 0);
        if (!t1.mLS0.empty()) {
          if (t1.mLS0.head()->action(*t1.mLS0.head()->_uarch,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(21+t0.mID.pos(0)) : 0);
        Logger *logger1 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(21+t0.mID.pos(1)) : 0);
        Logger *logger2 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(21+t0.mID.pos(2)) : 0);
        Logger *logger3 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(21+t0.mID.pos(3)) : 0);
        if (!t0.mID.empty()) {
          if (t0.mID.head()->action(*t0.mID.head()->_uarch,logger0,0)) {
            if (!t0.mID.empty()) {
              if (t0.mID.head()->action(*t0.mID.head()->_uarch,logger1,1)) {
              } else {
                _stall = true;
              }
            }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(25+t1.mID.pos(0)) : 0);
        Logger *logger1 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(25+t1.mID.pos(1)) : 0);
        Logger *logger2 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(25+t1.mID.pos(2)) : 0);
        Logger *logger3 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(25+t1.mID.pos(3)) : 0);
        if (!t1.mID.empty()) {
          if (t1.mID.head()->action(*t1.mID.head()->_uarch,logger0,0)) {
            if (!t1.mID.empty()) {
              if (t1.mID.head()->action(*t1.mID.head()->_uarch,logger1,1)) {
              } else {
                _stall = true;
              }
            }
  ...
  _stage_loggers(29), 
  top(*this), 
  t0(id, thread_id, out, startTime, mCX0, mCX1, mCX2, Mem_obj, L1d_obj, L1i_obj, BARs, BootAddr, CBTEST2, CCR, CfxIndex, FOOs, HID0, MSRs, NIAs, TCR, TEN, FwdCB, _stall, *this,static_cast<ISS_PMT_Thread*>(_iss->t0)), 
  t1(id, thread_id, out, startTime, mCX0, mCX1, mCX2, Mem_obj, L1d_obj, L1i_obj, BARs, BootAddr, CBTEST2, CCR, CfxIndex, FOOs, HID0, MSRs, NIAs, TCR, TEN, FwdCB, _stall, *this,static_cast<ISS_PMT_Thread*>(_iss->t1))
  {
    Mem_obj.set_level(0);
    Mem_obj.set_level(0);
  };
