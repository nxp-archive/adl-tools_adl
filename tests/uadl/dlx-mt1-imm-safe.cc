...
#  define ADL_HEADER
#endif
#include "dlx-mt1-imm-safe-iss.cc"
#define _OSM_Thread_IMPLEMENTATION_H
#include "dlx-mt1-imm-safe.h"
...
namespace uadl {
  ...
  void L1d_cache_t::reset_has_data()
  {
    if (_uarch.hadReset()) {
      ...
      unsigned num_sets = _uarch._iss->t0->L1d_perf_cache_get_sets();
      unsigned num_ways = _uarch._iss->t0->L1d_perf_cache_get_ways();
      _hasData = HasData(num_sets,BVect(num_ways,false));
      for (unsigned set = 0; set < num_sets; set++) {
        for (unsigned way = 0; way < num_ways; way++) {
          _hasData[set][way] = _uarch._iss->t0->L1d_perf_cache._array[set][way].valid;
        }
      }
    }
  }
  ...
  {
    const bool hit = _uarch._iss->t0->L1d_perf_cache_find(adl::CacheLogRead, (int&)set, (int&)way, addr);
    if (hit) offset = _uarch._iss->t0->L1d_perf_cache_get_offset(addr);
    return hit;
  }
  
  addr_t L1d_cache_t::getTagAddr(unsigned set, unsigned way)
  {
    return _uarch._iss->t0->L1d_perf_cache_get_tag(set, way);
  }
  ...
  void L1d_cache_t::operationBegin(uadl::MemoryRequest &request)
  {
    ...
    if (_uarch._iss->t0->L1d_perf_cache_begin(
    ...
    {
      request.offset = _uarch._iss->t0->L1d_perf_cache_get_offset(request.addr);
    } else {
  ...
  void L1d_cache_t::operationEnd(const uadl::MemoryRequest &request) const
  {
    if (request.action != adl::CacheNone) {
      _uarch._iss->t0->L1d_perf_cache_end(request.type,request.action, request.set, request.way,request.addr, hasCommand());
    }
  }
  ...
  {
    _uarch._iss->t0->L1d_perf_cache._array[set][way].valid = false;
  }
  ...
  void L1i_cache_t::reset_has_data()
  {
    if (_uarch.hadReset()) {
      // set up data array status if this is the first time step
      unsigned num_sets = _uarch._iss->t0->L1i_perf_cache_get_sets();
      unsigned num_ways = _uarch._iss->t0->L1i_perf_cache_get_ways();
      _hasData = HasData(num_sets,BVect(num_ways,false));
      for (unsigned set = 0; set < num_sets; set++) {
        for (unsigned way = 0; way < num_ways; way++) {
          _hasData[set][way] = _uarch._iss->t0->L1i_perf_cache._array[set][way].valid;
        }
      }
    }
  }
  ...
  {
    const bool hit = _uarch._iss->t0->L1i_perf_cache_find(adl::CacheLogRead, (int&)set, (int&)way, addr);
    if (hit) offset = _uarch._iss->t0->L1i_perf_cache_get_offset(addr);
    return hit;
  }
  
  addr_t L1i_cache_t::getTagAddr(unsigned set, unsigned way)
  {
    return _uarch._iss->t0->L1i_perf_cache_get_tag(set, way);
  }
  ...
  void L1i_cache_t::operationBegin(uadl::MemoryRequest &request)
  {
    ...
    if (_uarch._iss->t0->L1i_perf_cache_begin(
            request.type, request.action,
            request.hint._cacheStatus,
            request.set, request.way, request.addr, request.replaceLine,
            hasCommand()
    ...
    {
      request.offset = _uarch._iss->t0->L1i_perf_cache_get_offset(request.addr);
    } else {
      request.action = adl::CacheNone;
    }
    request.lookupDone = true;
  ...
  {
    if (request.action != adl::CacheNone) {
      _uarch._iss->t0->L1i_perf_cache_end(request.type,request.action, request.set, request.way,request.addr, hasCommand());
    }
  }
  ...
  {
    _uarch._iss->t0->L1i_perf_cache._array[set][way].valid = false;
  }
