...
#  define ADL_HEADER
#endif
#include "dlx2-bp3-safe-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx2-bp3-safe.h"
...
namespace uadl {
  ...
  // Operands:
  //   (const: 1)
  struct bl_branch_LinkStackUpdate {
    bool can_read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      return true;
    }
  ...
  //
  template <class LinkStackUpdate_t, class Src_t, class Trg_t>
  struct Inst_branch : public Inst_P {
    
    typedef struct Inst_branch<LinkStackUpdate_t, Src_t, Trg_t> ThisClass;
    
    Inst_branch &thisInstr() { return *this; };
    
    LinkStackUpdate_t LinkStackUpdate;
    Src_t Src;
    Trg_t Trg;
    ...
    {
      LinkStackUpdate.allocate(uarch,ic,logger);
    ...
    {
      LinkStackUpdate.deallocate(uarch,ic,logger);
    ...
    {
      LinkStackUpdate.write(uarch,ic,logger);
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read_fp(uarch,*this,logger, uarch.FwdEX , uarch.FwdMM ) && Trg.can_write(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) ) {
            ...
            write_ops(uarch,*this,logger) ;
             ...
             bool mispredict = ! uarch.last_branch_prediction_correct(*this  ) ;
            if ( mispredict ) {
                  uarch.taken_flush_mID(*this,logger,position  ) ;
              }
            if ( LinkStackUpdate.value(uarch,*this,logger) ) {
                  uarch.bp.update_link_type(uarch,*this,logger) ;
              } else {
                  uarch.bp.update_normal(uarch,*this,logger) ;
              }
  ...
  //
  // Operands for instruction class branch_return
  //
  ...
  //
  // Instruction class branch_return
  //
  template <class Src_t, class Trg_t>
  struct Inst_branch_return : public Inst_P {
    
    typedef struct Inst_branch_return<Src_t, Trg_t> ThisClass;
    
    Inst_branch_return &thisInstr() { return *this; };
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read_fp(uarch,*this,logger, uarch.FwdEX , uarch.FwdMM ) && Trg.can_write(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) ) {
            ...
            uarch.bp.update_return_type(uarch,*this,logger) ;
            return change_state_S_ID__S_EX(uarch,logger,position) ;
      }
      return false;
      HandleException: {
  ...
  struct rfci_sfx_Src {
    ...
    bool can_read_fp(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0,T1 &t1) {
      {
        RegId id = static_cast<OSM_P&>(uarch).CSRR0.id(static_cast<OSM_P&>(uarch),0);
        tag_t tag = (static_cast<OSM_P&>(uarch).CSRR0.last_allocator( static_cast<OSM_P&>(uarch),0));
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || t1.can_read(uarch,ic,logger,id,tag) || uarch.CSRR0.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      {
        RegId id = static_cast<OSM_P&>(uarch).CSRR1.id(static_cast<OSM_P&>(uarch),0);
        tag_t tag = (static_cast<OSM_P&>(uarch).CSRR1.last_allocator( static_cast<OSM_P&>(uarch),0));
        if  (!(t0.can_read(uarch,ic,logger,id,tag) || t1.can_read(uarch,ic,logger,id,tag) || uarch.CSRR1.can_alloc(uarch,ic,logger,"read"))) return false;
      }
      return true;
    }
  ...
  struct Inst_sfx : public Inst_P {
    ...
    // State action S_INIT
    bool action_S_INIT(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mID.has_space(uarch,*this,logger) && ! uarch.mID.has_attr(uarch,*this,logger, 0 , adliss::P::muldiv_instrs ) ) {
            return change_state_S_INIT__S_ID(uarch,logger,position) ;
      }
      return false;
    }
  ...
  unsigned OSM_P::getInstrClassAttrByName(const std::string &attr)
  {
    ...
    if (attr == "return_type") return 0x8;
    if (attr == "serialized_instrs") return 0x10;
    if (attr == "sfx_instrs") return 0x20;
    if (attr == "special_instrs") return 0x40;
    if (attr == "store_instrs") return 0x80;
    return 0;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_branch<DummyOp, b_branch_Src, b_branch_Trg> *ic = (Inst_branch<DummyOp, b_branch_Src, b_branch_Trg>*)_instr_pool.alloc();
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_branch<DummyOp, bc_branch_Src, bc_branch_Trg> *ic = (Inst_branch<DummyOp, bc_branch_Src, bc_branch_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::branchClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_branch_return<bclr_branch_return_Src, bclr_branch_return_Trg> *ic = (Inst_branch_return<bclr_branch_return_Src, bclr_branch_return_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::branch_returnClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_branch<bl_branch_LinkStackUpdate, bl_branch_Src, bl_branch_Trg> *ic = (Inst_branch<bl_branch_LinkStackUpdate, bl_branch_Src, bl_branch_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::branchClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_sfx<rfci_sfx_Src, rfci_sfx_Trg> *ic = (Inst_sfx<rfci_sfx_Src, rfci_sfx_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::sfxClass,p);
    return ic;
  }
  ...
  bp(*this,0,BPredEnableDefault(),32,2,3), 
