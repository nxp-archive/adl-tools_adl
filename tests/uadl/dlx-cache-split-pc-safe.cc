...
#  define ADL_HEADER
#endif
#include "dlx-cache-split-pc-safe-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-cache-split-pc-safe.h"
...
namespace uadl {
  ...
  struct Inst_load : public Inst_P {
    ...
    // State action S_IDp
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.L1d.can_request_read(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) ) {
            uarch.exec_mem(*this,logger) ;
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym1039[0],&_sym1039[19]),
      0  // with-data
    ),
    _uarch(uarch)
  {
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym1040[0],&_sym1040[19]),
      0  // with-data
    ),
    _uarch(uarch)
  {
