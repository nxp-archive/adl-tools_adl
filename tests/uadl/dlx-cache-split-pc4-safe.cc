...
#  define ADL_HEADER
#endif
#include "dlx-cache-split-pc4-safe-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-cache-split-pc4-safe.h"
...
namespace uadl {
  ...
  struct Inst_branch : public Inst_P {
    ...
    bool change_state_S_EX0__S_EX1(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EX0 -> S_EX1"); }
      _action = (StateFunc)action_S_EX1_p;
      _flush = (FlushFunc)flush_S_EX1_p;
      uarch.mEX0.remove_instr();
      uarch.mEX1.add_instr(this);
      return true;
    }
    
    bool change_state_S_EX1__S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EX1 -> S_MM"); }
      _action = (StateFunc)action_S_MM_p;
      _flush = (FlushFunc)flush_S_MM_p;
      uarch.mEX1.remove_instr();
    ...
    bool change_state_S_IDp__S_EX0(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_IDp -> S_EX0"); }
    ...
    bool action_S_EX0(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX1.has_space(uarch,*this,logger) ) {
            return change_state_S_EX0__S_EX1(uarch,logger,position) ;
      }
      return false;
    }
    ...
    // State action S_EX1
    bool action_S_EX1(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) ) {
            return change_state_S_EX1__S_MM(uarch,logger,position) ;
      }
      return false;
    }
    static bool action_S_EX1_p(Inst_P *x,OSM_P &uarch,Logger *logger,unsigned position) {
      return static_cast<ThisClass*>(x)->action_S_EX1(uarch,logger,position);
    }
    ...
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX0.has_space(uarch,*this,logger) ) {
            return change_state_S_IDp__S_EX0(uarch,logger,position) ;
      }
      return false;
    }
    ...
    // Flush action S_EX1
    void flush_S_EX1(OSM_P &uarch,Logger *logger) {
      deallocate_ops(uarch,*this,logger);
    }
    static void flush_S_EX1_p(Inst_P *x,OSM_P &uarch,Logger *logger) {
      return static_cast<ThisClass*>(x)->flush_S_EX1(uarch,logger);
    }
  ...
  struct Inst_cache : public Inst_P {
    ...
    bool change_state_S_EX0p__S_EX1(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EX0p -> S_EX1"); }
      _action = (StateFunc)action_S_EX1_p;
      _flush = (FlushFunc)flush_S_EX1_p;
      uarch.mEX0.remove_instr();
      uarch.mEX1.add_instr(this);
    ...
    bool change_state_S_EX0p__S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EX0p -> S_MM"); }
      _action = (StateFunc)action_S_MM_p;
      _flush = (FlushFunc)flush_S_MM_p;
      uarch.mEX0.remove_instr();
      uarch.mMM.add_instr(this);
    ...
    // State action S_EX0
    bool action_S_EX0(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX1.has_space(uarch,*this,logger) && can_request_cmd(uarch, thisInstr (  ) , logger ) ) {
    ...
    bool action_S_EX0p(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) ) if ( exec_done (  ) && uarch.mEX1.has_space(uarch,*this,logger) ) {
            return change_state_S_EX0p__S_MM(uarch,logger,position) ;
      } else if ( uarch.L1d.can_request_cmd(uarch,*this,logger) ) {
            uarch.L1d.send_cmd(uarch,*this,logger) ;
            uarch.exec_mem(*this,logger) ;
            if ( uarch.mEX1.has_space(uarch,*this,logger) ) {
                  return change_state_S_EX0p__S_EX1(uarch,logger,position) ;
              }
      }
      return false;
    }
    ...
    // State action S_ID
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read(uarch,*this,logger) && uarch.mEX0.has_space(uarch,*this,logger) ) {
    ...
    // State action S_INIT
    bool action_S_INIT(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mID.empty(uarch,*this,logger) && uarch.mEX0.empty(uarch,*this,logger) && uarch.mMM.empty(uarch,*this,logger) && uarch.mWB.empty(uarch,*this,logger) ) {
            return change_state_S_INIT__S_ID(uarch,logger,position) ;
      }
      return false;
    }
  ...
  struct Inst_lmw : public Inst_P {
    ...
    // State action S_EX0
    bool action_S_EX0(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX1.has_space(uarch,*this,logger) && uarch.L1d.can_request_read(uarch,*this,logger) ) {
            uarch.exec_mem(*this,logger) ;
  ...
  struct Inst_load : public Inst_P {
    ...
    // State action S_ID
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read(uarch,*this,logger) && Trg.can_write(uarch,*this,logger) && uarch.mEX0.has_space(uarch,*this,logger) ) {
            Src.read(uarch,*this,logger) ;
            Trg.allocate(uarch,*this,logger) ;
            return change_state_S_ID__S_IDp(uarch,logger,position) ;
      }
    ...
    // State action S_IDp
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch. OSM_P::check_cache ( OSM_P::tFoo , thisInstr (  ) , logger ) && uarch.mEX0.has_space(uarch,*this,logger) ) {
            uarch.exec_mem(*this,logger) ;
  ...
  struct Inst_sfx : public Inst_P {
    ...
    bool change_state_S_EX1__S_WB(OSM_P &uarch,Logger *logger,unsigned position) {
      if (logger && uarch.logOsm()) { logger->logMessage("S_EX1 -> S_WB"); }
      _action = (StateFunc)action_S_WB_p;
      _flush = (FlushFunc)flush_S_WB_p;
      uarch.mEX1.remove_instr();
      uarch.mWB.add_instr(this);
    ...
    bool action_S_EX1(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mWB.has_space(uarch,*this,logger) ) {
            return change_state_S_EX1__S_WB(uarch,logger,position) ;
      }
      return false;
    }
    static bool action_S_EX1_p(Inst_P *x,OSM_P &uarch,Logger *logger,unsigned position) {
  ...
  struct Inst_stmw : public Inst_P {
    ...
    // State action S_EX0
    bool action_S_EX0(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX1.has_space(uarch,*this,logger) && uarch.L1d.can_request_write(uarch,*this,logger) ) {
            uarch.exec_mem(*this,logger) ;
  ...
  struct Inst_store : public Inst_P {
    ...
    // State action S_IDp
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.L1d.can_accept_write_requests(uarch,*this,logger) && uarch.mEX0.has_space(uarch,*this,logger) ) {
  ...
  void OSM_P::exec_mEX1(Inst_P&instr,Logger *logger,unsigned position)
  {
    exec(instr,logger);
  }
  
  void OSM_P::exec_mem_mEX1(Inst_P&instr,Logger *logger,unsigned position)
  {
    exec_mem(instr,logger);
  }
  ...
  void OSM_P::flush_mEX1(Inst_P&ic,Logger *logger,unsigned position)
  {
    _speculative = false;
    if (ic.iid() >= _resetFetchId) {
    }
    taken_flush_mEX1(ic,logger,position);
  }
  
  void OSM_P::taken_flush_mEX1(Inst_P&ic,Logger *logger,unsigned position,bool given_addr,addr_t addr)
  {
    _fetchException = 0;
    if (ic.iid() >= _resetFetchId) {
      if (!given_addr && ic._instr) {
  ...
  void OSM_P::update_tracing()
  {
    ...
    if (logPipeline()) {
      ...
      mEX1.createLogger(_logclient,tflags);
      _stage_loggers.set(2,mEX1.logger(0));
      mEX0.createLogger(_logclient,tflags);
      _stage_loggers.set(3,mEX0.logger(0));
      mID.createLogger(_logclient,tflags);
      _stage_loggers.set(4,mID.logger(0));
    } else {
      mWB.removeLogger();
      mMM.removeLogger();
      mEX1.removeLogger();
  ...
  void OSM_P::breakDeadlock()
  {
    if (!_failOnDeadlock) {
      ...
      for (unsigned i = 0; i != 1; ++i) {
        Logger *logger = ((Tracing && logPipeline()) ? _stage_loggers.get(1+mMM.headp()) : 0);
        if (Inst_P *instr = mMM.head()) {
          if (logger) logger->logMessage("discarding instruction to break deadlock.");
          instr->flush(*this,logger);
          mMM.remove_instr();
          discard(instr);
      ...
      for (unsigned i = 0; i != 1; ++i) {
        Logger *logger = ((Tracing && logPipeline()) ? _stage_loggers.get(2+mEX1.headp()) : 0);
        if (Inst_P *instr = mEX1.head()) {
          if (logger) logger->logMessage("discarding instruction to break deadlock.");
          instr->flush(*this,logger);
          mEX1.remove_instr();
          discard(instr);
      ...
      for (unsigned i = 0; i != 1; ++i) {
        Logger *logger = ((Tracing && logPipeline()) ? _stage_loggers.get(3+mEX0.headp()) : 0);
        if (Inst_P *instr = mEX0.head()) {
          if (logger) logger->logMessage("discarding instruction to break deadlock.");
          instr->flush(*this,logger);
          mEX0.remove_instr();
          discard(instr);
      ...
      for (unsigned i = 0; i != 1; ++i) {
        Logger *logger = ((Tracing && logPipeline()) ? _stage_loggers.get(4+mID.headp()) : 0);
        if (Inst_P *instr = mID.head()) {
          if (logger) logger->logMessage("discarding instruction to break deadlock.");
          instr->flush(*this,logger);
          mID.remove_instr();
          discard(instr);
  ...
  void OSM_P::cycle()
  {
    try {
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(2+mEX1.pos(0)) : 0);
        if (!mEX1.empty()) {
          if (mEX1.head()->action(*this,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(3+mEX0.pos(0)) : 0);
        if (!mEX0.empty()) {
          if (mEX0.head()->action(*this,logger0,0)) {
          } else {
            _stall = true;
          }
        }
      ...
      {
        Logger *logger0 ATTRIBUTE_UNUSED = ((Tracing && logPipeline()) ? _stage_loggers.get(4+mID.pos(0)) : 0);
        if (!mID.empty()) {
