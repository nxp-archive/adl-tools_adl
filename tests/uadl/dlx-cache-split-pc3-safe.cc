...
#  define ADL_HEADER
#endif
#include "dlx-cache-split-pc3-safe-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-cache-split-pc3-safe.h"
...
namespace uadl {
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( can_read_ops(uarch,*this,logger) && can_write_ops(uarch,*this,logger) ) {
            ...
            if ( ! uarch.last_branch_prediction_correct(*this  ) ) {
                  uarch.Fetcher.set_cancel(uarch,*this,logger, false ) ;
  ...
  // Operand Alias:  ori_sfx_Trg -> oris_sfx_Trg
  //   (trg: GPR(RA))
  typedef ori_sfx_Trg oris_sfx_Trg;
  ...
  MyCache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym1041[0],&_sym1041[21]),
      0  // with-data
    ),
    _uarch(uarch)
  {
  ...
  bool L1d_cache_t::storeBufEnabled()
  {
    return _uarch._iss->_sym410();
  }
  ...
  static CacheConfigMap::value_type _sym1042[] = {
    CacheConfigMap::value_type("critical_word_first",CacheConfigItem(1)),
    CacheConfigMap::value_type("evict_lock_cycle",CacheConfigItem(1)),
    CacheConfigMap::value_type("invalidate_latency",CacheConfigItem(1)),
    CacheConfigMap::value_type("linefill_access_width",CacheConfigItem(64)),
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym1042[0],&_sym1042[19]),
      0  // with-data
    ),
    _uarch(uarch)
  {
  ...
  void OSM_P::breakDeadlock()
  {
    if (!_failOnDeadlock) {
      ...
      for (unsigned i = 0; i != 1; ++i) {
        Logger *logger = ((Tracing && logPipeline()) ? _stage_loggers.get(2+mEX.headp()) : 0);
        if (Inst_P *instr = mEX.head()) {
          if (logger) logger->logMessage("discarding instruction to break deadlock.");
          instr->flush(*this,logger);
          mEX.remove_instr();
          discard(instr);
      ...
      for (unsigned i = 0; i != 1; ++i) {
        Logger *logger = ((Tracing && logPipeline()) ? _stage_loggers.get(3+mID.headp()) : 0);
