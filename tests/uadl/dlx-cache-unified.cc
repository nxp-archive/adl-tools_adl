...
#  define ADL_HEADER
#endif
#include "dlx-cache-unified-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-cache-unified.h"
...
namespace uadl {
  ...
  struct Inst_load : public Inst_P {
    ...
    // State action S_EX
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.L1.can_request_read(uarch,*this,logger) ) {
            uarch.exec_mem(*this,logger) ;
            uarch.L1.send_read_request(uarch,*this,logger) ;
    ...
    // State action S_IDp
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mEX.has_space(uarch,*this,logger) && check_latency(logger) ) {
            return change_state_S_IDp__S_EX(uarch,logger,position) ;
      } else {
            decr_latency (  ) ;
      }
    ...
    // State action S_MM
    bool action_S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.L1.can_read(uarch,*this,logger) ) {
            uarch.exec_and_read(*this,logger, uarch.L1 ) ;
    ...
    bool action_S_MMp(OSM_P &uarch,Logger *logger,unsigned position) {
      ...
      } else if ( uarch.L1.can_request_read(uarch,*this,logger) ) {
            uarch.L1.send_read_request(uarch,*this,logger) ;
  ...
  struct Inst_sfx : public Inst_P {
    ...
    // State action S_ID
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read(uarch,*this,logger) && Trg.can_write(uarch,*this,logger) && uarch.L1.request_queue_empty(uarch,*this,logger) ) {
            Src.read(uarch,*this,logger) ;
            Trg.allocate(uarch,*this,logger) ;
            return change_state_S_ID__S_IDp(uarch,logger,position) ;
      }
  ...
  struct Inst_store : public Inst_P {
    ...
    // State action S_EX
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.L1.can_request_write(uarch,*this,logger) ) {
            uarch.exec_mem(*this,logger) ;
            uarch.L1.send_write_request(uarch,*this,logger) ;
            return change_state_S_EX__S_MM(uarch,logger,position) ;
      }
      return false;
    }
    ...
    // State action S_MM
    bool action_S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.L1.can_write(uarch,*this,logger) ) {
            uarch.exec_and_write(*this,logger, uarch.L1 ) ;
            return change_state_S_MM__S_MMp(uarch,logger,position) ;
      }
      return false;
    }
    ...
    bool action_S_MMp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( exec_done (  ) && uarch.mWB.has_space(uarch,*this,logger) ) {
            return change_state_S_MMp__S_WB(uarch,logger,position) ;
      } else if ( uarch.L1.can_request_write(uarch,*this,logger) ) {
            uarch.L1.send_write_request(uarch,*this,logger) ;
            return change_state_S_MMp__S_MM(uarch,logger,position) ;
      }
      return false;
  ...
  static CacheConfigMap::value_type _sym921[] = {
  ...
  L1_cache_t::L1_cache_t(OSM_P &uarch, UadlMemoryIf &memory) :
  Cache(
      uarch,
      "L1",
      memory,
      ...
      1, // allow bus wait
      1, // pre-emptive
      CacheConfigMap(&_sym921[0],&_sym921[15]),
      1  // with-data
    ),
    _uarch(uarch)
  {
  ...
  void L1_cache_t::reset_has_data()
  {
    if (_uarch.hadReset()) {
      // set up data array status if this is the first time step
      unsigned num_sets = _uarch._iss->L1_cache_get_sets();
      unsigned num_ways = _uarch._iss->L1_cache_get_ways();
      _hasData = HasData(num_sets,BVect(num_ways,false));
      for (unsigned set = 0; set < num_sets; set++) {
        for (unsigned way = 0; way < num_ways; way++) {
          _hasData[set][way] = _uarch._iss->L1_cache._array[set][way].valid;
        }
      }
    }
  }
  
  bool L1_cache_t::lookup(addr_t addr, unsigned &set, unsigned &way, unsigned &offset) const
  {
    const bool hit = _uarch._iss->L1_cache_find(adl::CacheLogRead, (int&)set, (int&)way, addr);
    if (hit) offset = _uarch._iss->L1_cache_get_offset(addr);
    return hit;
  }
  
  byte_t* L1_cache_t::cachePosition(unsigned set, unsigned way, unsigned offset) const
  {
    return _uarch._iss->L1_cache_data_array(set, way) + offset;
  }
  
  addr_t L1_cache_t::getTagAddr(unsigned set, unsigned way)
  {
    return _uarch._iss->L1_cache_get_tag(set, way);
  }
  
  void L1_cache_t::resize() {
    unsigned num_sets = _uarch._iss->L1_cache_get_sets();
    unsigned num_ways = _uarch._iss->L1_cache_get_ways();
    _hasData = HasData(num_sets,std::vector<bool>(num_ways));
  }
  
  void L1_cache_t::operationBegin(uadl::MemoryRequest &request)
  {
    Cache::operationBegin(request);
    
    if (_uarch._iss->L1_cache_begin(
    ...
    {
      request.offset = _uarch._iss->L1_cache_get_offset(request.addr);
    } else {
  ...
  void L1_cache_t::operationEnd(const uadl::MemoryRequest &request) const
  {
    if (request.action != adl::CacheNone) {
      _uarch._iss->L1_cache_end(request.type,request.action, request.set, request.way,request.addr, hasCommand());
    }
  }
  
  
  void L1_cache_t::clearValid(unsigned set, unsigned way)
  {
    _uarch._iss->L1_cache._array[set][way].valid = false;
  }
  
  void L1_cache_t::preemptFetch()
  {
    _uarch.Fetcher.preemptFetch();
  }
  
  bool L1_cache_t::storeBufEnabled()
  {
    return _uarch._iss->_sym347();
  }
  
  bool L1_cache_t::is_write_through(addr_t addr)
  {
    return _uarch._iss->L1.is_write_through(adl::CacheWrite,addr);
  }
  ...
  void OSM_P::getMemoryData(CacheAccess type, addr_t addr, unsigned size, byte_t *data)
  {
    if (type == adl::CacheLogRead) {
      ...
      if (L1_obj.getMemoryData(type, addr, size, data)) {
        ...
        L1_obj.getMemoryData(type, addr, size, data);
      }
    } else {
      L1_obj.getMemoryData(type, addr, size, data);
    }
  ...
  void OSM_P::cancel_all_mem(Inst_P &instr)
  {
    ...
    while (ma) {
      if (ma->valid_request_id()) {
        L1_obj.cancel(ma->request_id(),true);
      }
      ma = ma->_next;
    }
  }
  ...
  void OSM_P::update_tracing()
  {
    ...
    if (logMemory()) {
      L1_obj.createLogger(_logclient,tflags);
    } else {
      L1_obj.removeLogger();
    }
    if (logMemory()) {
  ...
  void OSM_P::postCycle()
  {
    ...
    if (logMemory()) L1_obj.logAllRequests();
    if (tracing_on() && _out) {
  ...
  SRR1("SRR1"), 
  XER("XER"), 
  Fetcher(*this,getPath(_iss,".")+"Fetcher",16,L1_obj,0,_fetchSizes,&mMEM_obj), 
  _cur_instr(0), 
  _ignore_mem(false), 
  Issue("         "), 
  _fetchException(0), 
  ...
  bool OSM_P::active() const
  {
    ...
    return (_iss->activeCount() || instr_alloc_count() || mMEM_obj.isActive() || L1_obj.isActive());
  }
  ...
  unsigned OSM_P::dmemStalls() const { return L1_obj.memStalls(0); }
  
  unsigned OSM_P::imemStalls() const { return L1_obj.memStalls(0); }
