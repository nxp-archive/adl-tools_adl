...
#  define ADL_HEADER
#endif
#include "dlx-bubble-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-bubble.h"
...
namespace uadl {
  ...
  struct Inst_branch : public Inst_P {
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && ! uarch.stalled() ) {
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( can_read_ops(uarch,*this,logger) && can_write_ops(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) && ! uarch.stalled() ) {
    ...
    bool action_S_MM(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mWB.has_space(uarch,*this,logger) && ! uarch.stalled() ) {
  ...
  struct Inst_load : public Inst_P {
    ...
    // State action S_EX
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.mMEM.can_request_read(uarch,*this,logger) && ! uarch.stalled() ) {
    ...
    // State action S_ID
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( Src.can_read(uarch,*this,logger) && Trg.can_write(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) && ! uarch.stalled() ) {
    ...
    bool action_S_MMp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( exec_done (  ) && uarch.mWB.has_space(uarch,*this,logger) && ! uarch.stalled() ) {
  ...
  //
  // Operands for instruction class spr
  //
  
  // Operands:
  //   (src: GPR(RS))
  struct mtspr_spr_Src {
    bool can_read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      if (!uarch.GPR.can_alloc(uarch,ic,logger,ic.op(0)._index,"read")) return false;
      return true;
    }
  ...
  //
  // Instruction class spr
  //
  template <class Src_t, class Trg_t>
  struct Inst_spr : public Inst_P {
    
    typedef struct Inst_spr<Src_t, Trg_t> ThisClass;
    
    Inst_spr &thisInstr() { return *this; };
    ...
    bool action_S_ID(OSM_P &uarch,Logger *logger,unsigned position) {
      {
            if ( Src.can_read(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) && ! uarch.stalled() ) {
                  Src.read(uarch,*this,logger) ;
                  Trg.allocate(uarch,*this,logger) ;
                  return change_state_S_ID__S_EX(uarch,logger,position) ;
              }
  ...
  struct Inst_store : public Inst_P {
    ...
    // State action S_EX
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.mMEM.can_request_write(uarch,*this,logger) && ! uarch.stalled() ) {
            uarch.exec_mem(*this,logger) ;
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_spr<mtspr_spr_Src, mtspr_spr_Trg> *ic = (Inst_spr<mtspr_spr_Src, mtspr_spr_Trg>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::sprClass,p);
    return ic;
  }
  ...
  NIA("NIA"), 
  XER("XER"), 
  fetcher(*this,getPath(_iss,".")+"fetcher",4,mMEM_obj,0,_fetchSizes,&mMEM_obj), 
  _cur_instr(0), 
  _ignore_mem(false), 
  Issue("         "), 
  _stall(false), 
