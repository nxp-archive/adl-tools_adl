    ...
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0,T1 &t1) {
      {
        ...
        if (t1.can_read_partial(uarch,ic,logger,static_cast<OSM_P&>(uarch).GPR.id(static_cast<OSM_P&>(uarch),ic.op(0)._index)).first != 0) {
  ...
  struct or_sfx_Src {
    ...
    void read(OSM_P &uarch,Inst_P &ic,Logger *logger,T0 &t0,T1 &t1) {
      ...
      }
      {
        ...
        if (t1.can_read_partial(uarch,ic,logger,static_cast<OSM_P&>(uarch).GPR.id(static_cast<OSM_P&>(uarch),ic.op(2)._index)).first != 0) {
          t1.read_partial(uarch,ic,logger,*this,1);
          if (logger) log_read(uarch,ic,logger,"read from "+t1.name(),1);
        }
      }
