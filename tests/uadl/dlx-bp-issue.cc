...
#  define ADL_HEADER
#endif
#include "dlx-bp-issue-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-bp-issue.h"
  ...
  struct Inst_branch : public Inst_P {
    ...
    bool action_S_INIT(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mID.has_space(uarch,*this,logger) ) {
             AdlTypeof(uarch.BP.predict(uarch,*this,logger)) p = uarch.BP.predict(uarch,*this,logger) ;
            if ( p . first == Taken ) {
                  uarch.taken_flush_Issue(*this,logger,position ,  true , p . second ) ;
              }
