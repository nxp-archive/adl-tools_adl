...
#  define ADL_HEADER
#endif
#include "dlx-cache-split-safe-ext-mem-iss.cc"
#define _OSM_P_IMPLEMENTATION_H
#include "dlx-cache-split-safe-ext-mem.h"
...
void adliss::P::handle_memory_latency(unsigned memId, CacheAccess type, addr_t addr, unsigned size, const MemHandler::Latencies &latency)
{
  if (!latency.empty()) {
    uadl::UadlMemoryIf &mem = _uarch->Mem_obj;
    
    static_cast<uadl::ExtLatencyMem&>(mem).addLatency(type, addr, size, latency);
    _memhandler_mem->clear_latency();
  }
}
...
namespace uadl {
  ...
  //
  // Operands for instruction class data_cache
  //
  
  // Operands:
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  struct dcbt_data_cache_Src {
    bool can_read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
  ...
  // Operand Alias:  dcbt_data_cache_Src -> dcbf_data_cache_Src
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  typedef dcbt_data_cache_Src dcbf_data_cache_Src;
  
  // Operand Alias:  dcbt_data_cache_Src -> dcbst_data_cache_Src
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  typedef dcbt_data_cache_Src dcbst_data_cache_Src;
  
  // Operand Alias:  dcbt_data_cache_Src -> dcbz_data_cache_Src
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  typedef dcbt_data_cache_Src dcbz_data_cache_Src;
  
  // Operand Alias:  dcbt_data_cache_Src -> dcbi_data_cache_Src
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  typedef dcbt_data_cache_Src dcbi_data_cache_Src;
  
  // Operand Alias:  dcbt_data_cache_Src -> dcba_data_cache_Src
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  typedef dcbt_data_cache_Src dcba_data_cache_Src;
  
  //
  // Instruction class data_cache
  //
  template <class Src_t>
  struct Inst_data_cache : public Inst_P {
    
    typedef struct Inst_data_cache<Src_t> ThisClass;
    
    Inst_data_cache &thisInstr() { return *this; };
    ...
    // State action S_EX
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.L1d.can_request_cmd(uarch,*this,logger) ) {
  ...
  //
  // Operands for instruction class instr_cache
  //
  
  // Operands:
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  struct icbtls_instr_cache_Src {
    bool can_read(OSM_P &uarch,Inst_P &ic,Logger *logger) {
      if (!uarch.GPR.can_alloc(uarch,ic,logger,ic.op(0)._index,"read")) return false;
      if (!uarch.GPR.can_alloc(uarch,ic,logger,ic.op(1)._index,"read")) return false;
      return true;
  ...
  // Operand Alias:  icbtls_instr_cache_Src -> icblc_instr_cache_Src
  //   (src: GPR(RA))
  //   (src: GPR(RB))
  typedef icbtls_instr_cache_Src icblc_instr_cache_Src;
  
  //
  // Instruction class instr_cache
  //
  template <class Src_t>
  struct Inst_instr_cache : public Inst_P {
    
    typedef struct Inst_instr_cache<Src_t> ThisClass;
    
    Inst_instr_cache &thisInstr() { return *this; };
    ...
    bool action_S_EX(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.mMM.has_space(uarch,*this,logger) && uarch.L1d.can_request_cmd(uarch,*this,logger) ) {
            uarch.exec_mem(*this,logger) ;
            uarch.L1i.send_cmd(uarch,*this,logger) ;
            uarch.exec_mem(*this,logger) ;
            return change_state_S_EX__S_EXp(uarch,logger,position) ;
      }
      return false;
  ...
  struct Inst_load : public Inst_P {
    ...
    // State action S_IDp
    bool action_S_IDp(OSM_P &uarch,Logger *logger,unsigned position) {
      if ( uarch.L1d.can_accept_read_requests(uarch,*this,logger) && uarch.mEX.has_space(uarch,*this,logger) ) {
            return change_state_S_IDp__S_EX(uarch,logger,position) ;
      }
      return false;
    }
  ...
  static CacheConfigMap::value_type _sym1039[] = {
  ...
  Cache(
      ...
      0, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym1039[0],&_sym1039[12]),
      0  // with-data
    ),
    _uarch(uarch)
  {
  ...
  void L1d_cache_t::reset_has_data()
  {
    if (_uarch.hadReset()) {
      ...
      unsigned num_sets = _uarch._iss->L1d_perf_cache_get_sets();
      unsigned num_ways = _uarch._iss->L1d_perf_cache_get_ways();
      _hasData = HasData(num_sets,BVect(num_ways,false));
      for (unsigned set = 0; set < num_sets; set++) {
        for (unsigned way = 0; way < num_ways; way++) {
          _hasData[set][way] = _uarch._iss->L1d_perf_cache._array[set][way].valid;
        }
      }
    }
  }
  ...
  {
    const bool hit = _uarch._iss->L1d_perf_cache_find(adl::CacheLogRead, (int&)set, (int&)way, addr);
    if (hit) offset = _uarch._iss->L1d_perf_cache_get_offset(addr);
    return hit;
  }
  
  addr_t L1d_cache_t::getTagAddr(unsigned set, unsigned way)
  {
    return _uarch._iss->L1d_perf_cache_get_tag(set, way);
  }
  ...
  void L1d_cache_t::operationBegin(uadl::MemoryRequest &request)
  {
    ...
    if (_uarch._iss->L1d_perf_cache_begin(
    ...
    {
      request.offset = _uarch._iss->L1d_perf_cache_get_offset(request.addr);
    } else {
  ...
  void L1d_cache_t::operationEnd(const uadl::MemoryRequest &request) const
  {
    if (request.action != adl::CacheNone) {
      _uarch._iss->L1d_perf_cache_end(request.type,request.action, request.set, request.way,request.addr, hasCommand());
    }
  }
  ...
  {
    _uarch._iss->L1d_perf_cache._array[set][way].valid = false;
  }
  ...
  Cache(
      ...
      1, // allow bus wait
      0, // pre-emptive
      CacheConfigMap(&_sym1040[0],&_sym1040[12]),
      0  // with-data
    ),
    _uarch(uarch)
  {
  ...
  void L1i_cache_t::reset_has_data()
  {
    if (_uarch.hadReset()) {
      // set up data array status if this is the first time step
      unsigned num_sets = _uarch._iss->L1i_perf_cache_get_sets();
      unsigned num_ways = _uarch._iss->L1i_perf_cache_get_ways();
      _hasData = HasData(num_sets,BVect(num_ways,false));
      for (unsigned set = 0; set < num_sets; set++) {
        for (unsigned way = 0; way < num_ways; way++) {
          _hasData[set][way] = _uarch._iss->L1i_perf_cache._array[set][way].valid;
        }
      }
    }
  }
  ...
  {
    const bool hit = _uarch._iss->L1i_perf_cache_find(adl::CacheLogRead, (int&)set, (int&)way, addr);
    if (hit) offset = _uarch._iss->L1i_perf_cache_get_offset(addr);
    return hit;
  }
  
  addr_t L1i_cache_t::getTagAddr(unsigned set, unsigned way)
  {
    return _uarch._iss->L1i_perf_cache_get_tag(set, way);
  }
  ...
  void L1i_cache_t::operationBegin(uadl::MemoryRequest &request)
  {
    ...
    if (_uarch._iss->L1i_perf_cache_begin(
            request.type, request.action,
            request.hint._cacheStatus,
            request.set, request.way, request.addr, request.replaceLine,
            hasCommand()
    ...
    {
      request.offset = _uarch._iss->L1i_perf_cache_get_offset(request.addr);
    } else {
      request.action = adl::CacheNone;
    }
    request.lookupDone = true;
  ...
  {
    if (request.action != adl::CacheNone) {
      _uarch._iss->L1i_perf_cache_end(request.type,request.action, request.set, request.way,request.addr, hasCommand());
    }
  }
  ...
  {
    _uarch._iss->L1i_perf_cache._array[set][way].valid = false;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_data_cache<dcba_data_cache_Src> *ic = (Inst_data_cache<dcba_data_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::data_cacheClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_data_cache<dcbf_data_cache_Src> *ic = (Inst_data_cache<dcbf_data_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::data_cacheClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_data_cache<dcbi_data_cache_Src> *ic = (Inst_data_cache<dcbi_data_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::data_cacheClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_data_cache<dcbst_data_cache_Src> *ic = (Inst_data_cache<dcbst_data_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::data_cacheClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_data_cache<dcbt_data_cache_Src> *ic = (Inst_data_cache<dcbt_data_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::data_cacheClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_data_cache<dcbz_data_cache_Src> *ic = (Inst_data_cache<dcbz_data_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::data_cacheClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_instr_cache<icblc_instr_cache_Src> *ic = (Inst_instr_cache<icblc_instr_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::instr_cacheClass,p);
    return ic;
  }
  ...
  {
    InstrPacket *p ATTRIBUTE_UNUSED = (InstrPacket*)(x);
    Inst_instr_cache<icbtls_instr_cache_Src> *ic = (Inst_instr_cache<icbtls_instr_cache_Src>*)_instr_pool.alloc();
    ic->reset(_issueId++,_commits,adliss::P::instr_cacheClass,p);
    return ic;
  }
  ...
  SRR1("SRR1"), 
  XER("XER"), 
  Fetcher(*this,getPath(_iss,".")+"Fetcher",1,L1i_obj,0,_fetchSizes,&Mem_obj), 
  _cur_instr(0), 
  _last_instr(0), 
  _speculative(false), 
  _branch_target(0), 
